...
- Read the whole project goal from a compass in the header, on the board and on a card page alike, and edit it there: `kanban-ui/README.md`.
...
- Start a release from the header's release dropdown — it is on every board, including one that has never planned a version, and a name the board can't take says why without closing the dialog; renaming and reordering a release stay terminal jobs: `kanban-ui/README.md`.
...
- The New release dialog carries a fill toggle — on by default, stating the rule (every unplanned high-priority card goes in) rather than a count, with the cards it would put in named under it and the ones it would leave named with the test each failed: `kanban-ui/README.md`.
- Give up on a version from the ⋯ menu beside the header's release dropdown — Drop shows while that release is on screen, and a confirm dialog lists the open cards losing their release before anything is written: `kanban-ui/README.md`.
- Close a shipped version from that same ⋯ menu — the confirm says what goes down as shipped, lists the open cards coming out of the version, and names any card whose todos are all ticked but which was never archived while you can still cancel and archive it; the board goes back to All releases after: `kanban-ui/README.md`.
- Run a recurring card from its page — **Run** stands in for Implement, does one pass of the card's Process, records the run, and leaves the card on the board; the page says when the job last ran, Archive and Refine never show, and the recurring column carries its own faint tint: `kanban-ui/README.md`.
- Write the goal into an empty box with the ask and a link to the guide above it, instead of deleting a seeded paragraph first — and saving stops the board asking for a goal right away, without waiting on an agent run: `kanban-ui/README.md`.
- Give a recurring card a cadence on its page — a number, a unit, and a time of day for whole days, with "No cadence" on the same list — and the board runs that job itself when it comes due, showing the next run beside the last one; the runs land in the runs panel and the Auto-refine switch has no say over them: `kanban-ui/README.md`.
