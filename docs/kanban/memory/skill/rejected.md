...
- **Moving Codex's `--sandbox` after its `resume` subcommand** — before it, Codex honours it;
  other products need `-c sandbox_mode=` only because they put it after.
- **`--skip-git-repo-check` on Codex, and `--trust` on Cursor** — neither is needed: the board
  and every worktree are git repos, and a headless Cursor run works untrusted.
...
- **An image input for the ZCode harness** — every GLM model in ZCode's config declares
  text-only input, so the picture is dropped before the provider call. Declaring it would swap
...
- **`propose`, a flow that finds new work from the board alone** — guessing what to build from
  memory made cards nobody could trace back to anything. Finding new work reads a real source.
