// Layout C — a named panel under the meta band, one row per card this one is
// holding up, with its title. Card page of the board UI, 1280x800.
...
// The panel — the same row the subtasks panel already draws: id, title, chevron.
function HoldingUp() {
  const rows = [
    { id: 201, title: "Order the queue by what it unblocks" },
    { id: 202, title: "Put the release picker at the top of the board" },
    { id: 233, title: "Show a card's spec agents on the board" },
  ];
  return (
    <div className="mb-4 rounded-[14px] border-[1.5px] bg-white p-3" style={{ borderColor: INK }}>
      <div className="mb-2 inline-flex items-center gap-2 text-[11px] font-[700] uppercase leading-none tracking-[0.12em]">
        <Lock open color="#dd4f1e" />
        holding up
        <span className="tabular-nums" style={{ color: SOFT }}>3</span>
      </div>
      <ul className="flex flex-col gap-1.5">
        {rows.map((r) => (
          <li key={r.id} className="flex items-center gap-2.5 rounded-[10px] px-2.5 py-2">
            <span className="shrink-0 text-[12px] font-[800]" style={{ color: DEEP }}>#{r.id}</span>
            <span className="min-w-0 flex-1 truncate text-[13.5px] font-[700] leading-snug">{r.title}</span>
            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke={SOFT} strokeWidth="2.2"
              strokeLinecap="round" strokeLinejoin="round" style={{ flex: "0 0 auto" }}>
        ...
        <div className="min-w-0 flex-1 pl-1">
          <div className="h-full overflow-hidden rounded-tl-[14px] bg-white">
            <main className="mx-auto w-full max-w-[840px] px-6 py-6">
              ...
              {/* the meta band — the link columns stay ids; the list gets a panel */}
