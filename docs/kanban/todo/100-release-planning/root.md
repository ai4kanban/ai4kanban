...
- A card names the release it ships in. A card that names none is in no release: wanted,
  not promised to a version. Shipped.
...
- Closing a release is the user's call, not something that happens on its own. The close
  writes down what shipped, clears the release off the cards still open, and takes the
  release off the list. Shipped.
- A card's release can be changed from the UI, the way its priority can. Shipped.
- Several cards can be ticked and moved into a release, or back out of it, in one
  action (#114).
- Board and queue view get one dropdown: which release am I looking at. Shipped.
...
- A card made from the board while one release is picked lands in that release, so new work
  is not off the screen the moment it is written. Shipped.
- Making a release can put the high-priority cards into it on the spot, so a version does
  not start empty and nobody moves them in one at a time. Shipped.
- A release the team gives up on can be dropped: `release drop` takes it off the list,
  clears the release off its open cards, and records the drop so nothing ever reads as
  shipped — from the terminal or the UI. Shipped.
- Closing a release works from the UI too, with the confirm showing which open cards
  lose their release — the close itself shipped, but only in the terminal (#136).
- Order: the `release` field on a card came first — every other piece is about that field.
  ...
  anything, and it is where the UI first reads the release list. Then picking one release
  to look at, on that same list. Those five have shipped, and so have the fill, off the
  New release dialog that shipped with #115, and the drop. #114 needs nothing more and
  can land any time. #136 fronts a close that already shipped, so it too can land any
  time.
...
- [x] Pick which release the board and queue view show #104
- [ ] Move several cards into a release at once #114
- [x] Make a release from the UI #115
- [x] Fill a new release with the high-priority cards #106
- [x] Drop a release that will not ship #131
- [ ] Close a release from the UI #136
...
- **No release is an empty field, not a named state.** The board first shipped with a
  `next` sentinel for unplaced cards; the user overruled it — a card with no release just
  has an empty field, and nothing anywhere is called `next`.
...
- **A new card follows the release on screen.** The other option is landing every new card
  in no release, out of sight of the person who just wrote it. Propose is different: it
  offers work nobody has planned, so its cards start with no release.
- **One piece owns each job, so no two repeat it.** The field work kept the local UI from
  ...
  lives. The card page is where the UI reads the release list, and the release dropdown and
  #114 use that same list. The card page moves one card, #114 moves the cards the user
  picked, the fill picks the cards itself.