...
work that way. Add the two controls that make a board stricter: a board-wide setting that
requires approval of the exact reviewed tree before landing, and a `critical` label that requires
it on one card plus every check the flow rules ask for.

## Worth noting
- Off by default. Requiring diff approval on every card puts the user back in the loop for every
  change, which is what auto-delivery exists to remove.
- An approval is for one exact tree. A rebase, a correction, or any later change to the tree
  cancels it and asks again — otherwise "approved" would stop meaning anything.
- A `critical` card is stricter without changing the board's default. That is the point: strict
...
- **`critical` label**: a card labelled `critical` requires diff approval and every check the
  flow rules require, whatever the board-level setting says.
- A routine card keeps the default four-part review — task summary, worth noting, open questions,
  worth noting after implementation.
- The card page is where an approval is given, next to the diff #305 shows.
- Record each approval, what tree it covered, and each cancellation on the run's audit record.
...
- [ ] Record approvals, the tree each covered, and cancellations on the audit record.
