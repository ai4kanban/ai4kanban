...
- Read #326's machine-local session and add no second sign-in: the app and `akb` act as the
  same account, and a machine nobody has opened the app on is told to sign in there rather
  than offered a flow of its own (#311).
...
- [ ] Read #326's session from `akb` and the app, and tell a machine that holds none to sign
      in from the app's Configuration dialog.
