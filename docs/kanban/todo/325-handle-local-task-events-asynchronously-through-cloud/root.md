...
questions:
  - question: "[user] Cloud keeps every event, delivery attempt, action and outcome in a free-tier Supabase project with no backups, and no card says for how long one is kept. #321's privacy page has to state the answer before the first invite goes out. How long does Cloud keep a finished event's history?"
    mode: single
    options:
...
- **The board stays local**: Cloud carries authenticated event snapshots and human actions,
  while the local board, repository, and code stay on the execution node.
- **Approval stays explicit**: reaching `ready` creates a review request, and approval of that
  exact revision starts implementation.
...
- **The desktop inbox comes first**: it proves messages and actions as the core flow; Slack
  follows as the first external connector.
...
- **Why a group and not one card**: each of the four pieces is too large for one run, and none
  of them is worth building without the others.
...
- **Why the Todo runs in that order**: identity first, because every later row hangs off an
  ...
  re-renders a flow that already works.
...
- **Why #327 is not in this group**: it replaces #326's hand-written admission with a code the
  invited person redeems. It ships in the same release and sits behind #326, but nothing in
  the event, action, or execution flow reads it, and this group is finished without it.
...
- #321 and #327 — the privacy and terms pages, and the invitation code, both shipping in
  0.8.0 beside this group.
