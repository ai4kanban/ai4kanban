...
// ai4kanban install` with. So the app carries that same package — ../cli's bin/ and dist/ —
// and runs it under Electron's own Node (see lib/board-init.js).
//
...
// Exactly what the published tarball carries: bin/, dist/, and the
// package.json the CLI reads its own version out of.
fs.cpSync(path.join(cli, "bin"), path.join(to, "bin"), { recursive: true });
fs.cpSync(path.join(cli, "dist"), path.join(to, "dist"), { recursive: true });
