...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* A recent project whose board lives in a Cloud workspace. */
  .pill {
    margin-left: 7px;
    padding: 1px 7px;
    border-radius: 999px;
...
function script(c: DesktopCopy["launcher"]): string {
  ...
  function row(p) {
    ...
    name.append(p.name);
    // A Cloud board opens the same way and behaves differently once open, so the row says
    // which it is.
    if (p.cloud) {
      const badge = document.createElement("span");
      badge.className = "pill";
      badge.textContent = ${JSON.stringify(c.cloudBadge)};
      name.append(badge);
    }
