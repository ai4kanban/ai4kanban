...
export function serializeFrontmatter(m: CardMeta): string {
  ...
  // lib/cadence.ts). Written only when the card carries one; no cadence means the
...
export function parseFrontmatter(text: string): ParsedCard {
  ...
  const normalized: CardMeta = {
    ...
    // (lib/cadence.ts); a line that isn't one of the accepted forms means the
    // card has no working cadence and only runs by hand.
