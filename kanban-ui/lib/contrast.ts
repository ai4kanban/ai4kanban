// WCAG 2.1 contrast, so app/design can print a number instead of leaving "is
// this readable?" to whoever is looking at it. Used only by that page — it is
// prerendered at build time, so nothing here rides along in a board bundle.
//
// Kept in step with web/lib/contrast.ts, which does the same job for the site.
 ...
 *  The board leans on `color-mix` for its hairlines and washes, so several pairs
 *  can only be measured after they are flattened. */
...
const THRESHOLD: Record<TextSize, number> = {
  // Normal-weight copy, and anything under 18.66px bold / 24px regular — which
  // is every size the board sets, its 10.5px bold chips included.
  body: 4.5,
