// How often a recurring card repeats (#139), ported verbatim from
// skill/lib/cadence.mjs so the script and this server read a card's `cadence`
// the same way.
//
...
export function nextDue(lastRun: string, cadence: string | Cadence | null): Date | null {
  const c = typeof cadence === "string" || cadence === null ? parseCadence(cadence) : cadence;
  if (!c) return null;
  ...
  const [hour, minute] = c.at
    ? c.at.split(":").map(Number)
