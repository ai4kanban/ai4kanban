...
// The market signals waiting to be looked at (#453) — what `akb signals fetch` pulled into
...
// posts, and a rail row is a couple of hundred pixels wide. A page rather than a pane, so
// Back, Forward and a reload keep you where you were.
//
// Three things happen here and nothing else: read a signal, open the post it came from, and
// ignore it. A signal is a lead, not a card — nothing on this page creates one, ranks one, or
// touches the board's counts. Turning a signal into a card is #454's.

import { useCallback, useEffect, useRef, useState } from "react";
import { FiChevronDown, FiChevronRight, FiExternalLink, FiGlobe, FiTrash2 } from "react-icons/fi";
import {
  SiReddit,
  SiSinaweibo,
  SiX,
...
import { dismissSignalAction } from "@/app/actions";
...
import { PanelAction } from "./button";
...
// --- the marks the platforms are known by ------------------------------------
// A small table, matched on the `platform` field as it was sent. The pull side knows no
// platform at all — `platform` is free text, so any endpoint connects — and the price of that
// is a generic mark for anything not in this table.
...
function PlatformMark({ platform }: { platform: string }) {
  const known = MARKS.find((mark) => mark.match.test(platform));
...
export function SignalsPage({
  ...
  // Not configured is not an empty inbox: one is a board with nothing to say yet, the other
  // is a board that was never pointed anywhere. They read differently or the second one looks
  // like the first and nobody goes looking for the setting.
...
/** One signal: what it says, where it came from, and the three things you may do with it.
 *
 *  The summary is folded by default — a page of full posts is a page nobody scans — and
 *  **View summary** is what opens this one. Dismissing takes no confirmation and offers no
 *  undo: the page says so once at the top, and a lead is not worth a dialog. */
