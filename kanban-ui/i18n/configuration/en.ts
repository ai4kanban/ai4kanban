...
const en: ConfigurationCopy = {
  ...
  specSkills: {
    title: "Spec skills",
    blurb:
      "Choose which specialists may add focused recommendations while a card is being planned. They never run while a card is being built. Add your own in docs/kanban/skills/.",
    loading: "Loading spec skills…",
    tooOld:
      "The board's rules in this project are too old to list the spec skills. Update the command and reopen this dialog.",
    enabled: "Enabled",
    paused: "Paused",
    problems: "Not usable, so not listed:",
    switchOn: (skill) => `${skill} — enabled`,
    switchOff: (skill) => `${skill} — paused`,
    contributes: "Contributes",
    runsWhen: "Runs when",
    change: "Change",
    setting: (label, value) => `${label}: **${value}**`,
    settingWithCost: (label, value, cost) => `${label}: **${value}** — ${cost}`,
    flipFailedOn: (skill) => `couldn't switch ${skill} on`,
    flipFailedOff: (skill) => `couldn't switch ${skill} off`,
    saveFailed: (skill) => `couldn't save ${skill}'s setting`,
  },
  delivery: {
