...
const zh: ConfigurationCopy = {
  ...
  title: "配置",
  sections: "配置分区",
  navGroup: { settings: "设置", customize: "定制" },
  section: {
    general: "通用",
    runtimes: "运行时",
    workflows: "工作流",
  ...
  workflows: {
    ...
    builtIn: "内置",
    isDefault: "默认",
    builtInNames: { coding: "软件开发", "hyperframes-video": "产品视频", "slide-deck": "演示文稿" },
    builtInDescriptions: {
      coding: "规划、实现并审查软件改动。",
    ...
    helpers: "辅助 Agent",
    addHelper: "添加辅助",
    noneInStage: "这个环节还没有辅助 Agent。",
    emptyPage: "添加之后，可以在这里查看和修改它的说明与运行设置。",
    reviewers: "评审员",
    addReviewer: "添加评审员",
    noReviewers: "没有评审员时，构建完成直接交付。",
    worktree: "使用独立 Git 工作区",
    worktreeHint: "用于编码工作流：在独立分支上修改，完成后合入项目。",
    worktreeOn: "已启用",
    worktreeOff: "未启用",
    ...
    find: "查找 Agent…",
    noCandidates: "没有可用的 Agent",
    newAgent: "新建一个",
    newAgentHint: "新建的 Agent 只能做辅助或评审员",
    sharedWith: (flows) => `共用 · ${flows.join("、")}`,
    sharedTip: (flows) => `这里的修改也会改到「${flows.join("」「")}」`,
    yoursDivider: "我的",
    roleNote: "看板内置角色，职责说明由命令提供，不能修改。",
    deleteUsedBy: (flows) => `「${flows.join("」「")}」还在用它。`,
    more: (flow) => `工作流「${flow}」的更多操作`,
    duplicate: "复制工作流",
    rename: "重命名",
    remove: "删除",
    advanced: "高级设置",
    ...
    retired: "此工作流用到的部分 Agent 已停用，相关分配已移除。",
    retiredSeen: "知道了",
    loading: "正在加载工作流…",
  ...
  agents: {
    ...
    roles: {
      ...
      "code-reviewer": {
        ...
        note: "是否每次构建都评审，属于交付设置，在「通用 → 交付」里。",
      },
      gater: {
        name: "自动批准执行",
        gloss: "判断一张已就绪的卡片能不能无人值守地开发。",
        trigger: "卡片就绪时",
        rule: "会附加到它每次判断的末尾——例如「动到数据库的卡片一律不放行」。",
        when: "卡片进入「可以开发」时。批准的立刻开始执行，其余退回「未就绪」，并留下一个等你决定的问题。",
      },
      decider: {
        name: "自动回答问题",
        gloss: "替你回答卡片上等你决定的问题。",
        trigger: "只剩待答问题时",
        rule: "会附加到它每次作答的末尾——例如「拿不准就选改动最小的那个」。",
        when: "卡片上只剩等你决定的问题时，包括评审把交付打回后留下的。它作答后卡片继续往下走。",
        confirm: {
          title: "打开「自动回答问题」？",
      ...
      triage: {
        ...
        note: "不建议和「自动批准执行」「自动回答问题」同时开着：外部来的条目可能一条人工确认都没经过，就从待筛选一路做到实施。",
      },
    },
    decider: {
    ...
    sweeper: {
      ...
      sweeping: "正在整理",
      lastSweep: "上次整理",
      counts: (looked, kept, discarded) => `看了 ${looked} 张 · 保留 ${kept} 张 · 下板 ${discarded} 张`,
      viewReport: "查看报告",
      reportTitle: "整理报告",
      back: "返回",
      ...
      unfinished: "未完成",
      sat: (days) => `放了 ${days} 天`,
      stoppedHere: "整理停在这张卡片上，下个周期或点「立即整理」时继续。",
      openRun: "查看运行",
    },
    dismissalReviewer: {
      run: "立即回顾",
  ...
  privacy: {
    title: "使用情况上报",
    body: "上报匿名的功能使用与失败情况——只有次数与事件名称，不含你写下的任何文字。绝不包含代码、卡片内容、项目名称或文件路径。",
    feedback: "「反馈」不受此开关控制：只在你点击提交时发送，包含你写下的文字，关掉上报后照常送达。",
  ...
  partner: {
    title: "合作反馈",
    body: "分享问题时附带相关对话和代码，帮助我们分析需求理解偏差。",
    terms: "查看合作说明",
    consent: {
      title: "帮助我们改进 AI4Kanban",
      blurb: "当 AI 误解你的需求时，你可以分享相关材料，帮助我们改进需求规划。",
      choice: "每次是否分享，都由你在讨论中决定。",
      terms: [
        "相关对话和项目代码将分享给 AI4Kanban 团队，用于分析和改进。",
        "分析使用你当前配置的 AI 服务。",
        "已分享的材料会长期保留。",
      ],
      reversible: "你可以随时在设置中关闭此功能。",
      cancel: "取消",
      confirm: "开启反馈",
    },
    switchOn: (name) => `${name}已开启`,
  ...
  workspace: {
    ...
    leave: "离开这个工作区",
    leaveHint: "Cloud 表示这个工作区已不属于你的账号。摘掉指针后，docs/kanban/ 里现有的内容重新成为看板。",
    leaveButton: "离开…",
    leaveTitle: "确定离开这个工作区？",
    leaveBlurb: "没有内容可以写回，这一步只是让这个检出目录不再读取它。你的卡片都还在 git 历史里。",
    ends: "结束这个工作区",
    delete: "删除工作区",
  ...
  cloud: {
    ...
    storage: {
      ...
      checking: "正在检查当前存储位置…",
      where: "当前存储位置",
      whereCloud: "卡片、归档、记忆和看板设置都保存在你的 Cloud 工作区里。",
      whereLocal: "卡片、归档、记忆和看板设置都保存在这个检出目录里。",
      atCloud: (workspace) => `云端 · ${workspace}`,
      atCloudUnnamed: "云端",
      atLocal: "本地 · docs/kanban/",
      store: "把这个看板存到云端",
      storeHint: "开启后整个看板迁到一个新建的 Cloud 工作区；关闭后整份迁回 docs/kanban/。",
      storeLabel: "把这个看板存到云端",
      needsApp: "迁移在 AI4Kanban 应用里进行。请在应用里打开这个看板再切换存储位置。",
      tooOld: "这个项目使用的 akb 版本过旧，无法迁移看板。请先更新。",

      onTitle: "迁移到云端",
      onBlurb: "docs/kanban/ 里的全部内容——卡片、归档、记忆和看板设置——整份迁到一个新建的 Cloud 工作区。",
      workspaceName: "工作区名字",
      workspaceNameHint: "默认取项目文件夹名，可以改。每次都新建一个工作区，不会并入账号里已有的。",
      nameEmpty: "请为工作区取个名字。",
      onAction: "迁移到云端",

      offTitle: "迁回本地",
      offBlurb: (workspace) =>
        `工作区「${workspace}」里的全部内容——卡片、归档、记忆和看板设置——整份写回 docs/kanban/。`,
      offUnnamed: "云端工作区里的全部内容——卡片、归档、记忆和看板设置——整份写回 docs/kanban/。",
      offKeeps:
        "云端工作区保留不删，里面的东西一样不少，只是这个检出目录不再读取它。需要删除时，到「工作区」页另行操作。",
      offAction: "迁回本地",

      repoLead: "仓库里会留下这一条改动，读过再由你提交：",
      onUntrack: "docs/kanban/ 不再由 git 跟踪",
      onPointer: "新增 .ai4kanban.json",
      onIgnore: "根目录 .gitignore 增加一段",
      offTrack: "docs/kanban/ 重新由 git 跟踪",
      offPointer: "删除 .ai4kanban.json",
      offIgnore: "根目录 .gitignore 移除那一段",
      pauses: "迁移期间看板不可操作，结束后点「回到看板」即可继续，不需要重启应用或重开项目。",
      cancel: "取消",

      waitTitle: "正在等待运行中的任务结束",
      waitBlurb: "这些任务跑完就立刻开始迁移，期间写入的内容不会被落下。",
      waitRun: (job, card) => `${job} · ${card}`,
      movingOn: "正在把看板迁移到云端",
      movingOff: "正在把看板迁回这个检出目录",
      movingBlurb: "请保持这个页面打开。看板较大时需要一点时间。",
      doneOn: (workspace) => `看板现在保存在「${workspace}」里，这个检出目录从那里读取它。`,
      failedOn: "看板没有迁移到云端。",
      failedOff: "看板没有迁回本地。",
      failedBlurb: "什么都没有改变，看板仍停在迁移前的状态。处理好之后可以再试一次。",
      back: "回到看板",
      stop: "不迁移了",
    },
    ...
    notifications: {
      title: "这个看板",
      notify: "通知",
      notifyOn: "这个看板需要人时提醒我——已开启",
      notifyOff: "这个看板需要人时提醒我——已关闭",
      notifyBlurb: "在你打开这块板的每台机器上都是这个设置。你的队友照样会收到提醒。",
      watching: "跟踪",
      allReleases: "全部",
      pickRelease: "—— 选择一个版本 ——",
      ...
      sharedWatch: "在你打开这块板的每台机器上都是这个设置。",
      saveFailed: "保存失败",
    },
  },
};
