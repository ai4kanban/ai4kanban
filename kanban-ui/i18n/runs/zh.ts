...
const zh: RunsCopy = {
  ...
  dialog: {
    ...
    implement: {
      ...
      manualWhy: (why) =>
        `点一下就把这张卡片走完：Agent 先构建，再由一次全新的运行评审并修复，然后停下。当前是**手动提交模式**——${why}，因此不会替你提交：评审通过后由你自己提交，卡片会在那时归档。`,
      questionsOne:
        "这张卡片还有 **1 个待澄清问题**。它会被构建和评审，然后在合入前等你答复——也可以按**答复并开发**先把问题答掉。",
      questionsMany: (n) =>
        `这张卡片还有 **${n} 个待澄清问题**。它会被构建和评审，然后在合入前等你答复——也可以按**答复并开发**先把问题答掉。`,
      ...
      schedule: "加入计划",
      scheduleHint: "等没有任何阻碍后，自动构建这张卡片",
    },
    run: {
      title: (id) => `执行 #${id}`,
      blurb:
