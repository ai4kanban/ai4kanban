// 简体中文 —— the runs panel and everything an agent run says, mirroring `en.ts`
// key for key. Writing rules: `i18n/index.ts`.
import type { RunsCopy } from "./types";

const zh: RunsCopy = {
  action: {
    implement: "开发",
    gate: "把关",
    review: "评审",
    conflict: "解冲突",
    run: "执行",
    edit: "修改",
    clarify: "提问",
    resolve: "答复",
    decide: "代答",
    writing: "改写",
    reject: "否决",
    archive: "归档",
    create: "新建",
    propose: "提案",
    "plan-release": "规划版本",
    changelog: "更新日志",
    setup: "初始化",
    "prune-memory": "整理记忆",
    "review-memory": "回顾对话记忆",
    "review-dismissals": "回顾忽略记录",
    unstick: "清卡",
    reflect: "回顾",
    triage: "筛选条目",
    spec: "补充规格",
  },
  verb: {
    implement: "开发",
    gate: "把关",
    review: "评审",
    conflict: "解决冲突",
    run: "执行",
    edit: "修改",
    clarify: "提问",
    resolve: "答复",
    decide: "代答",
    writing: "改写",
    reject: "否决",
    archive: "归档",
    create: "新建",
    propose: "提案",
    "plan-release": "规划",
    changelog: "写更新日志",
    setup: "初始化",
    "prune-memory": "正在整理记忆",
    "review-memory": "正在回顾对话记忆",
    "review-dismissals": "正在回顾忽略记录",
    unstick: "正在了结搁置太久的卡片",
    reflect: "正在回顾已完成的任务",
    triage: "正在筛选待筛选条目",
    spec: "补充规格",
  },
  step: {
    implement: "开发",
    gate: "把关",
    review: "评审",
    conflict: "解冲突",
    run: "执行",
    edit: "修改",
    clarify: "提问",
    resolve: "答复",
    decide: "代答",
    writing: "改写",
    reject: "否决",
    archive: "归档",
    create: "新建",
    propose: "提案",
    "plan-release": "规划版本",
    changelog: "更新日志",
    setup: "初始化",
    "prune-memory": "整理记忆",
    "review-memory": "回顾对话记忆",
    "review-dismissals": "回顾忽略记录",
    unstick: "清卡",
    reflect: "回顾",
    triage: "筛选条目",
    spec: "补充规格",
  },
  flow: {
    edit: "修订",
    clarify: "澄清",
    writing: "澄清",
  },
  trigger: {
    conflict: "解决冲突之后",
    answered: "你回答之后",
    asked: "你要求的",
  },
  badge: {
    running: "运行中",
    watch: "查看运行日志",
    doing: (label) => `${label}——运行中`,
    idle: "Agent 运行中",
  },
  log: {
    title: "运行日志",
    expand: "展开运行日志",
    collapse: "收起运行日志",
    events: "中间事件",
    noOutput: "（无输出）",
    waiting: "…",
    stopped: "已停止",
    interrupted: "已中断",
    done: "已完成",
    failed: "失败",
    exitCode: (code) => `退出码 ${code}`,
    nothingDone: "未完成任何一步",
    blocked: "阻塞",
    running: "运行中",
    seconds: (s) => `${s} 秒`,
    minutes: (m, s) => `${m} 分 ${s} 秒`,
    hours: (h, m) => `${h} 时 ${m} 分`,
    costTiny: "约 <$0.01",
    cost: (usd) => `约 $${usd}`,
    costHint:
      "按标价用本次运行的 token 数折算。它表示这次运行如果按量购买要花多少钱，而不是你实际被扣的费用；订阅制下单次运行并不单独计费。",
    modelHint: "本次运行报告它所使用的模型。",
    tokens: (input, cacheWrite, cacheRead, output) =>
      `token · 输入 ${input} · 缓存写入 ${cacheWrite} · 缓存读取 ${cacheRead} · 输出 ${output}`,
    tokensHint: "本次运行的 token 数，来自 Agent 自己的报告：新增输入、提示缓存的写入与读取，以及输出。",
    stoppedShort: "这次运行中途停止，卡片可能只做了一半——它写下的内容都留在你的工作区里。",
    stoppedShortResume: "「继续」会从停下的地方接着做。",
    tickedNothing: "这次设置运行正常结束，但设置清单上没有勾掉任何一步。原因见下方 Agent 的回复。",
    blocker: {
      heading: "开发受阻",
      step: "步骤",
      cause: "原因",
      unblock: "如何继续",
    },
  },
  stop: {
    label: "中止运行",
    stopping: "中止中…",
    title: "中止这次运行",
    confirm: "中止这次运行？",
    body: "它会就地结束。写了一半的内容会留在你的工作区里。",
    failed: "未能中止这次运行",
  },
  retry: {
    waiting: (seconds, attempt, of) => `${seconds} 秒后重试 · 第 ${attempt}/${of} 次`,
    starting: (attempt, of) => `正在开始第 ${attempt}/${of} 次尝试`,
  },
  resume: {
    label: "继续",
    resuming: "继续中…",
    hint: "从这次运行失败的地方接着做——编码 Agent 会接回它自己的会话",
    failed: "未能继续这次运行",
  },
  panel: {
    open: "运行历史",
    openRunning: (n) => `${n} 个运行中——运行历史`,
    openUnhandled: (n) => `${n} 个运行未完成，待处理——运行历史`,
    heading: "运行",
    empty: "还没有任何运行。",
    pick: "选择一次运行，查看它的输入和日志。",
    note: "备注",
    resumedSession: "续接",
    cancelled: "已取消",
    stopped: (label) => label,
    kept: {
      tag: "成果已保留",
      blurb: "这次构建没有做完，它的工作目录和分支都还留着。「继续」从停下的地方接着做，「放弃」会把它们删掉。",
      carryOn: "继续",
      carryingOn: "继续中…",
      discard: "放弃",
      discarding: "放弃中…",
      carryOnFailed: "未能继续这次构建",
      discardFailed: "未能放弃这次构建",
    },
    steps: (n) => `${n} 个会话`,
    justNow: "刚刚",
    minutesAgo: (m) => `${m} 分钟前`,
    hoursAgo: (h) => `${h} 小时前`,
    daysAgo: (d) => `${d} 天前`,
  },
  scene: {
    office: "Agent 办公室",
    bot: (role, harness, task, state) => `${role}（${harness}）· ${task} · ${state}`,
    state: {
      running: "运行中",
      done: "已完成",
      error: "失败",
      interrupted: "已中断",
      stopped: "已停止",
    },
    noHarness: "未知 Agent",
    noRole: "Agent",
    running: (n) => `${n} 个运行中`,
    idle: "当前没有运行",
    runningTitle: "运行中",
    completed: "已完成",
    unfinished: "未完成",
    unfinishedCount: (n) => `未完成 ${n}`,
    collapse: "收起",
    page: (n, of) => `第 ${n} / ${of} 间`,
    prevRoom: "上一间",
    nextRoom: "下一间",
    unavailable: "此处无法绘制办公室。",
  },
  cardless: {
    planning: (release) => `正在规划${release}`,
    plan: (release) => `规划${release}`,
    writingChangelog: (release) => `正在为${release}写更新日志`,
    changelog: (release) => `更新日志${release}`,
    proposing: "正在提出任务",
    propose: "提出任务",
    finishingSetup: "正在完成初始化",
    finishSetup: "完成初始化",
    creating: "正在新建任务",
    create: "新建任务",
  },
  dialog: {
    cancel: "取消",
    implement: {
      title: (id) => `开发 #${id}`,
      autoBranch: (branch) =>
        `点一下就把这张卡片走完：Agent 先构建，再由一次全新的运行评审并修复，最后看板把它作为一个提交合入 \`${branch}\`。`,
      autoHere:
        "点一下就把这张卡片走完：Agent 先构建，再由一次全新的运行评审并修复，最后看板把它作为一个提交合入你当前所在的分支。",
      needsApproval: "在此之前它会等你批准工作区，因为这个看板要求差异批准。",
      thenArchives: "随后它会勾掉 todo、写下交付说明，并把卡片归档。",
      manualFolder:
        "点一下就把这张卡片走完：Agent 就在你的项目目录里构建，再由一次全新的运行评审并修复，然后停下。不会替你提交：评审通过后由你自己提交，卡片会在那时归档。",
      manual:
        "点一下就把这张卡片走完：Agent 先构建，再由一次全新的运行评审并修复，然后停下。当前是**手动提交模式**，因此不会替你提交：评审通过后由你自己提交，卡片会在那时归档。",
      manualWhy: (why) =>
        `点一下就把这张卡片走完：Agent 先构建，再由一次全新的运行评审并修复，然后停下。不会替你提交——${why}。评审通过后由你自己提交，卡片会在那时归档。`,
      ownBranch: "在单独的分支上构建",
      ownBranchOn: "Agent 在项目的一份独立副本（git worktree）里工作，你自己的文件原样不动。",
      ownBranchOff: "Agent 就在当前目录里工作，在你提交它留下的改动之前，别的构建都开不了。",
      autoBranchNoReview: (branch) =>
        `点一下就把这张卡片走完：Agent 构建之后，看板直接把它作为一个提交合入 \`${branch}\`，中间没有任何评审。`,
      autoHereNoReview:
        "点一下就把这张卡片走完：Agent 构建之后，看板直接把它作为一个提交合入你当前所在的分支，中间没有任何评审。",
      manualFolderNoReview:
        "点一下就把这张卡片走完：Agent 就在你的项目目录里构建，然后停下。不会有评审，也不会替你提交：由你自己提交它构建出来的改动，卡片会在那时归档。",
      manualNoReview:
        "点一下就把这张卡片走完：Agent 构建之后停下。不会有评审，而且当前是**手动提交模式**，因此不会替你提交：由你自己提交它构建出来的改动，卡片会在那时归档。",
      manualWhyNoReview: (why) =>
        `点一下就把这张卡片走完：Agent 构建之后停下。不会有评审，也不会替你提交——${why}。由你自己提交它构建出来的改动，卡片会在那时归档。`,
      files: "Agent 制作文件，经 AI 评审通过后归档任务。成品可在任务中查看。",
      filesNoReview: "Agent 制作文件，完成后归档任务。成品可在任务中查看。",
      recorded:
        "按下即刻记录。这里不会开始构建：卡片会一直等待机器，由你正在运行的任意一台机器接手，并把它走完。",
      questionsOne:
        "这张卡片还有 **1 个待澄清问题**。它会被构建，然后在合入前等你答复——也可以按**先答复**现在就把问题答掉。",
      questionsMany: (n) =>
        `这张卡片还有 **${n} 个待澄清问题**。它会被构建，然后在合入前等你答复——也可以按**先答复**现在就把问题答掉。`,
      ackQuestionsOne: "我知道还有一个问题没答复。",
      ackQuestionsMany: (n) => `我知道还有 ${n} 个问题没答复。`,
      blockedOne: (ids) => `这张卡片被 ${ids} 阻塞，它在看板上仍未完成。请先做完那张卡片。`,
      blockedMany: (ids) => `这张卡片被 ${ids} 阻塞，它们在看板上仍未完成。请先做完那些卡片。`,
      blockedOneSchedule: (ids) =>
        `这张卡片被 ${ids} 阻塞，它在看板上仍未完成。请先做完那张卡片——或者**加入计划**，等那张卡片做完，看板会自动开始构建。`,
      blockedManySchedule: (ids) =>
        `这张卡片被 ${ids} 阻塞，它们在看板上仍未完成。请先做完那些卡片——或者**加入计划**，等那些卡片做完，看板会自动开始构建。`,
      ackBlockedOne: (ids) => `我知道 ${ids} 还没做完。`,
      ackBlockedMany: (ids) => `我知道 ${ids} 还没做完。`,
      notReady:
        "这张卡片还没有标记为**待开发**，方案可能仍然粗糙。可以先在它的页面上按**澄清**，把它推到待开发。",
      ackNotReady: "我知道方案可能还很粗糙。",
      notes: "给 Agent 的补充说明（可选）…",
      confirm: "开发",
      confirmAnyway: "仍然开发",
      resolveFirst: "先答复",
      resolveFirstHint: "先答复待澄清的问题，再构建这张卡片",
      schedule: "加入计划",
      scheduleHint: "等没有任何阻碍后，自动构建这张卡片——按仓库的提交设置来，不看这里的勾选。",
    },
    run: {
      title: (id) => `执行 #${id}`,
      blurb:
        "Agent 按顺序走完这张卡片的**流程**，记录本次运行，并改写其中一两步，让下一次运行更少地需要你。卡片会留在看板上——周期任务永远不算完成。",
      unattended:
        "运行时没人盯着，因此需要你判断的步骤会被跳过，留作问题等你之后答复。",
      lastRun: (when) => `上次运行：${when}。`,
      neverRun: "这张卡片从未运行过。",
      notes: "本次运行的补充说明（可选）…",
      confirm: "执行",
    },
    refine: {
      title: (id) => `澄清 #${id}`,
      blurb:
        "Agent 把这张卡片往前推进一步：能自己定的问题它直接决策，只有你能决定的留给你，并把方案打磨得更清楚。它改的是卡片，不是代码。",
      blockedOne: (ids) => `这张卡片被 ${ids} 阻塞，它在看板上仍未完成。那张卡片做完后方案可能会变。`,
      blockedMany: (ids) =>
        `这张卡片被 ${ids} 阻塞，它们在看板上仍未完成。那些卡片做完后方案可能会变。`,
      blockedOneSchedule: (ids) =>
        `这张卡片被 ${ids} 阻塞，它在看板上仍未完成。那张卡片做完后方案可能会变——**加入计划**，等它离开看板，看板会自动澄清这张卡片。`,
      blockedManySchedule: (ids) =>
        `这张卡片被 ${ids} 阻塞，它们在看板上仍未完成。那些卡片做完后方案可能会变——**加入计划**，等它们离开看板，看板会自动澄清这张卡片。`,
      confirm: "澄清",
      confirmAnyway: "仍然澄清",
      schedule: "加入计划",
      scheduleHint: "等没有任何阻碍后，自动澄清这张卡片",
    },
    reject: {
      title: (id) => `移除 #${id}`,
      blurb: "这张卡片将移出看板。",
      placeholder: "否决原因（可选）…",
      hint: "留空仅丢弃，以后仍可重新提出；填写则否决，原因用于后续规划。",
      confirm: "否决",
      confirmDiscard: "丢弃",
    },
    archive: {
      title: (id) => `归档 #${id}`,
      blurb: "所有 todo 都已完成。卡片就此完成，移出看板。",
      placeholder: "给 Agent 的备注（可选）…",
      confirm: "归档",
    },
  },
};

export default zh;
