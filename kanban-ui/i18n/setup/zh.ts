...
const zh: SetupCopy = {
  ...
  project: {
    ...
    tracks: "工作分类",
    tracksBlurb:
      "每张卡片属于一个分类。分类名称将作为 `docs/kanban/todo/` 下的目录名。",
    trackName: (n) => `分类 ${n}`,
    trackNote: (track) => `${track} 的说明`,
    thisTrack: "这个分类",
    trackNotePlaceholder: "此分类包含的工作",
    dropTrack: (track) => `删除 ${track}`,
    dropTrackHint: "删除这个分类",
    trackLocked: (track) => `${track} 中还有卡片，请先移走`,
    addTrack: "添加分类",
    keptOne: (tracks) =>
      `已保存。${tracks} 中还有卡片，因此未删除该分类。请先移动或归档这些卡片。`,
    keptMany: (tracks) =>
      `已保存。${tracks} 中还有卡片，因此未删除这些分类。请先移动或归档这些卡片。`,
    saveFailed: "项目信息保存失败",
    continue: "继续",
  },
  goal: {
