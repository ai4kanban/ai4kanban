...
const en: RailCopy = {
  ...
  signals: {
    row: "Triage",
    title: "Triage",
    meta: (folder, count) => `${folder} · ${count === 1 ? "1 item" : `${count} items`}`,
    latestImport: (when) => `Newest ${when}`,
    lead: "Anything that might become work. Nothing here is scheduled or counted, and what you dismiss does not come back.",
    empty: "Nothing here yet. Drop a file or paste a link above.",
    connect: "Pull from an endpoint too",
    needEndpoint: (file) => `Triage endpoint — not set. Fill it in ${file}.`,
    needToken: (file) => `TRIAGE_ENDPOINT_TOKEN — not set. Fill it in ${file}.`,
    list: "Triage",
    viewSummary: "Read",
    hideSummary: "Hide",
    viewOriginal: "Open",
    dismiss: "Dismiss",
    dismissFailed: "could not dismiss that",
    add: {
      title: "Add to triage",
