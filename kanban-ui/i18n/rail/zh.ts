// 简体中文 —— the rail, the memory pages, the goal and Insights, mirroring `en.ts`
// key for key. Writing rules: `i18n/index.ts`.
import type { RailCopy } from "./types";

const zh: RailCopy = {
  search: "查找卡片",
  clearSearch: "清空搜索",
  matching: "匹配的卡片",
  allCards: "全部卡片",
  matches: "匹配结果",
  noMatches: "没有匹配的卡片。",
  runningRow: (label) => `${label}——运行中`,
  running: "运行中",
  discussions: {
    heading: "讨论",
    unnamed: "新讨论",
    answeringRow: (name) => `${name}——正在回复`,
    answering: "正在回复",
    menu: (name) => `对${name}的操作`,
    archive: "结束讨论",
    archiveFailed: "这段对话没能结束。",
    archiveNeedsCard: "打开这段讨论，选择要分享的卡片，或关闭分享。",
  },
  memory: {
    heading: "记忆",
    show: "看板和它的 Agent 记住了什么",
    hide: "隐藏记忆",
    board: "看板",
    planning: "规划",
    empty: "还没有记下任何内容。",
    files: {
      readme: "已交付的能力",
      goal: "目标",
      decisions: "已定下的决策",
      redesign: "设计上的教训",
      rejected: "被否决的想法",
      dismissed: "筛选偏好",
      feedback: "反馈",
    },
    prune: "整理记忆",
    pruneTitle: "整理记忆——在配置里打开它的页面",
  },
  archive: {
    row: "归档",
    title: "归档",
    meta: (folder, count) => `${folder} · ${count} 张卡片`,
    empty: "还没有归档任何卡片。卡片完成后会被移到这里，也在这里查看。",
    list: "已归档的卡片",
    undated: "在看板开始记录归档日期之前完成",
    card: { label: "归档", release: "版本", archived: "归档于" },
  },
  signals: {
    row: "待筛选",
    title: "待筛选",
    pending: "待处理",
    history: "历史",
    window: (days) => `最近 ${days} 天`,
    search: "搜索待筛选",
    allSources: "全部来源",
    hits: (shown, total) => `${shown} / ${total}`,
    noSource: "未注明来源",
    fold: "收起",
    unfold: "展开",
    more: "加载更多",
    empty: "暂无待筛选条目",
    emptyHint: "已完成卡片的后续事项和已接入来源的内容会自动出现在这里。",
    seeHistory: "查看历史",
    emptyHistory: "暂无历史",
    emptyHistoryHint: "最近 30 天生成卡片或被忽略的条目会出现在这里。",
    noHits: "没有匹配的条目",
    clear: "清除条件",
    connect: "从 endpoint 拉取",
    detail: "条目",
    collected: "采集于",
    dismissedAt: "忽略于",
    dismissedWhy: "理由",
    madeAt: "生成卡片于",
    madeInto: "卡片",
    byYou: "你忽略的",
    byAgent: "Agent 忽略的",
    contentGone: "原内容未保留",
    viewOriginal: "查看原文",
    makeCard: "生成卡片",
    making: "正在生成卡片…",
    makeFailed: "未能开始生成卡片，请重试。",
    ignore: "忽略",
    ignoreTitle: "忽略这条内容",
    why: "原因（可选）",
    reasonHint: "可简单写一句原因",
    dismissFailed: "忽略失败，请重试。",
    restore: "恢复",
    restoreFailed: "恢复失败。",
    sortAll: "全部分拣",
    sorting: "正在分拣…",
    sortClosed: "此看板暂不支持分拣。",
    sortRefused: "无法开始分拣，请检查 Agent 配置。",
    openConfig: "打开配置",
  },
  memoryPage: {
    unwritten: "这里还没有内容。随着看板被使用，它会逐渐充实。",
    menu: "对这个文件的操作",
    copyPath: "复制路径",
    copyRelative: "复制相对路径",
    copied: (what) => `${what}已复制`,
    path: "路径",
    relativePath: "相对路径",
  },
  goal: {
    open: "目标",
    openHint: "这个看板是为什么而存在的",
    write: "写下目标",
    writeHint: "写下这个看板为什么而存在——可选，随时可补",
    title: "目标",
    reading: "载入中…",
    editTitle: "填写目标",
    guideTitle: "什么样的目标是好目标",
    guideLine:
      "用你自己的话写项目要去往何处：你想要什么、大致多久、接下来大概做什么。写得粗略、简短都没关系，之后也可以改——Agent 绝不会替你起草目标。",
    saveFailed: "目标保存失败",
  },
  insights: {
    open: "统计",
    title: "统计",
    daily: {
      reading: "载入中…",
      empty: "还没有任何活动。你创建、归档或否决的卡片会出现在这里。",
      completed: "已完成",
      created: "已创建",
      rejected: "已否决",
      chart: (days) => `最近 ${days} 天的看板每日活动：完成、创建和否决的卡片数。`,
    },
    views: { trend: "趋势", activity: "活动" },
    periods: { "30d": "最近 30 天", "3m": "最近 3 个月", "6m": "最近 6 个月", "1y": "最近 1 年" },
    date: (m, d) => `${m} 月 ${d} 日`,
    week: (from, to) => `${from} – ${to}`,
    heat: {
      chart: "每日完成的卡片数",
      less: "少",
      more: "多",
      weekdays: ["一", "", "三", "", "五", "", "日"],
      month: (m) => `${m} 月`,
      tip: (date, n) => `${date} · 完成 ${n} 项`,
    },
    usage: {
      title: "用量",
      since: (date) => `${date}起有记录`,
      total: (usd) => `合计 约 $${usd}`,
      colSource: "连接器与模型",
      colTokens: "Token",
      colCost: "预估费用",
      costHint: "按 token 数和公开价格估算，不是账单；订阅套餐下单次运行不单独计费。",
      runs: (n) => `运行 ${n} 次`,
      turns: (n) => `对话 ${n} 轮`,
      unpriced: (n) => `其中 ${n} 次未报告费用`,
      noModel: "未记录模型",
      unknown: "来源未记录",
      empty: "这段时间没有运行或对话。",
    },
  },
};

export default zh;
