...
const ja: SiteCopy = {
  ...
  home: {
    meta: {
      ...
      description:
        "Claude Code のための AI プロジェクト管理。ぼんやりしたアイデアを渡すと、エージェントがそれを分解し、着手できる明確さになるまで問い直し続けます。ただの Markdown、git の中に。",
      social:
        "ぼんやりしたアイデアを渡してください。エージェントがそれを分解し、自分で決められることは自分で決め、残りをあなたに聞きます。そして細部まで着手できる明確さになるまで、裏で考え続けます。",
    },
    hero: {
      badge: "Claude Code のスキル",
      title: "あなたと一緒に育つ\nAI プロジェクト管理。",
      lead: "ぼんやりしたアイデアを渡してください。エージェントがそれを分解し、自分で決められることは自分で決め、残りをあなたに聞きます。そして細部まで着手できる明確さになるまで、裏で考え続けます。ボードの実体は `docs/kanban/` にあるただの Markdown。git の中にあり、データベースも MCP も要りません。",
    ...
    advanced: {
      ...
      metrics: {
        title: "タスクの指標",
        body: "アーカイブされたカード 1 枚がリリース 1 単位。だからあなたの速度は、作業のすぐ隣、git のなかの数字にすぎません。ダッシュボードも別ツールも不要です。",
        chart: {
  ...
  vsHermes: {
    ...
    summary: {
      ...
      when: "ボードを**コードと一緒にバージョン管理したい**とき、すでに動かしている環境の中に留まりたいとき、あるいはタスクボードのためだけにランタイムを運用したくないとき、このスキルを選んでください。**すでに Hermes を深く使っている**なら Hermes Kanban を。そのボードは、あなたが整えたディスパッチャ、名前付きプロファイル、チャットからの操作にそのまま噛み合います。結局どちらも堅牢なキューです。スキルのそれは git のファイル、Hermes のそれは SQLite の行というだけ。",
    },
    harness: {
      heading: {
        eyebrow: "実行環境の対応",
      ...
      lead: "いちばんはっきりした違いです。スキルのボードはただのファイルなので、**リポジトリを読めるエージェントならどれでも動かせます**。Hermes 自身も含めて。Hermes Kanban のボードはランタイムの `kanban_*` ツールの向こうにあるので、動かせるのは Hermes だけです。",
      oursSub: "ファイルを読めるエージェントなら何でも",
      theirsSub: "Hermes のみ",
      supported: "対応",
      notSupported: "非対応",
      note: "……そしてスキルの行はまだまだ続きます。Windsurf、OpenCode、Gemini CLI、ファイルを読めるものなら何でも。Hermes Kanban には他のエージェントの入り口がありません。",
    },
    comparison: {
      heading: { eyebrow: "真っ向比較", title: "AI4Kanban vs. Hermes Kanban" },
      lead: "{check} は明確な勝ち、**ダッシュ**はトレードオフ。スキルは簡潔さと可搬性で、Hermes は共有された堅牢なキューと規模で勝ちます。残りは引き分けです。",
      ourLabel: "AI4Kanban",
    ...
    memory: {
      ...
      lead: "本質的な違いは、スキルの記憶が**計画への入力**だという点です。次の提案を賢くするために存在します。Hermes のログは**実行からの出力**で、過去を再生できるように存在します。",
      ours: {
        heading: "AI4Kanban",
    ...
    gui: {
      ...
      lead: "どちらも Web ボードを備えていますが、役割が違います。スキルのボードは**エージェントを操る操作面**で、カードの操作が実行を起こします。Hermes のボードは**ディスパッチャへのライブな窓**で、フリートがいま何をしているかを映します。",
      ours: {
    ...
    wins: {
      ...
      theirs: {
        manyAgents: {
          ...
          body: "複数の名前付きエージェント、そして人がタスクを取り、作業を引き継ぐ、ひとつの堅牢なボード。ディスパッチャが着手可能なタスクを監視し、それぞれに割り当てられたエージェントを立ち上げます。スキルのボードは、あなたがいまいる 1 つの実行環境が回します。",
        },
        selfHealing: {
          title: "自己修復するタスクキュー",
          body: "キューはクラッシュをまたいで各タスクを追います。取得 TTL、ハートビート、期限切れ取得の回収、リトライ、サーキットブレーカー。ワーカーが途中で落ちてもボードが取り戻して再試行します。スキルのファイルも消えはしませんが、落ちた実行は次のスケジュールを待つだけです。",
        },
        autoDecompose: {
          title: "タスクを自動で分解する",
          body: "粗いタスクを放り込めば、ディスパッチャの LLM 分解器が子タスクのグラフに展開し、それぞれを専門エージェントに割り振ります。手作業の分解は不要です。スキルのほうは、カードを TODO と手入れするタスクグラフに分けます。",
        },
        fleetReach: {
          title: "フリート規模の到達範囲",
          body: "多数のボードにまたがる多数のエージェントのために作られ、マルチテナントで、Discord、Telegram、Slack、メール、SMS から操作できます。スキルのほうは、あなたのリポジトリとターミナルに留まる簡素なひとり用ボードです。",
        },
      },
    },
    decision: {
      ...
      note: "並べて使うこともできます。スキルは git のなかで計画し刈り込む軽い場所として、Hermes は何をやるか決まった後で重い共有作業を回す堅牢なキューとして。",
    },
  },
  ...
  vsVibe: {
    ...
    decision: {
      ...
      verdict:
        "埋めているボトルネックが違います。Vibe Kanban は多数のエージェントを回す**編成のコックピット**、ai4kanban は 1 体のエージェントがあなたのリポジトリで編集する**計画のボード**です。作業を並べる Vibe Kanban のボードが好きだったなら、このスキルはそれを、どんな会社より長生きするただのファイルとして渡します。並列エージェントのエンジンが好きだったなら、このスキルはそれではありません。はっきりそう言っておきます。",
