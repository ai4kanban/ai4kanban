...
const ja: VsHermesCopy = {
  ...
  memory: {
    ...
    ours: {
      ...
      body: "**意図的に刈り込まれた**小さなファイルが、2 つの層に分かれています。ボード自身の記録として `readme.md`（何をリリースしたか）と `goal.md`（プロジェクトの向かう先）があり、記憶するエージェントはそれぞれ自分のフォルダを持ちます。プランナーのフォルダには `decisions.md`（確定した判断とその理由）、`rejected.md`（何を、なぜ却下したか）、`redesign.md`（繰り返してはいけない設計ミス）が入ります。モジュールはこれらのファイル内の見出しであって、フォルダではありません。エージェントは提案やカード執筆の前にすべて読みます。完全な履歴は git の仕事です。",
      q: "案 X がボードに無いのはなぜ？",
