...
const ja: VsHermesCopy = {
  ...
  memory: {
    ...
    ours: {
      ...
      body: "小さなファイルが 4 つ、**意図的に刈り込まれています**。モジュールごとに 1 つのフォルダにまとめられ、`readme.md`（何をリリースしたか）、`decisions.md`（確定した判断とその理由）、`rejected.md`（何を、なぜ却下したか）、`redesign.md`（繰り返してはいけない設計ミス）を記録します。`goal.md` は記憶フォルダの最上位に単独で置かれます。エージェントは提案やカード執筆の前にすべて読みます。完全な履歴は git の仕事です。",
      q: "案 X がボードに無いのはなぜ？",
