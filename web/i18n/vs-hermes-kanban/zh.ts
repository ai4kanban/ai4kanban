...
const zh: VsHermesCopy = {
  ...
  memory: {
    ...
    ours: {
      ...
      body: "几份刻意保持精简的文件，分两层：看板自己的记录——`readme.md` 记已交付内容，`goal.md` 记项目方向；每个会记事的智能体各有一个文件夹，规划师的文件夹里 `decisions.md` 记已定下的决策及理由，`rejected.md` 记被否决的想法及理由，`redesign.md` 记需要避免的设计失误。模块是这些文件里的一个小标题，不是单独的文件夹。智能体会在提出或完善任务前读取这些文件，完整变更历史则由 git 保存。",
      q: "想法 X 为什么不在看板上？",
