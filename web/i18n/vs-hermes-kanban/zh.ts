...
const zh: VsHermesCopy = {
  ...
  memory: {
    ...
    ours: {
      ...
      body: "四个刻意保持精简的文件记录后续工作真正需要的信息：`archive.md` 记录已交付内容，`rejected.md` 记录被否决的想法及理由，`redesign.md` 记录需要避免的设计失误，`memory.md` 记录以往扫描得到的有效发现。智能体会在提出或完善任务前读取这些文件，完整变更历史则由 git 保存。",
      q: "想法 X 为什么不在看板上？",
