...
const ja: DownloadCopy = {
  ...
  builds: {
    title: "どのビルドを選ぶか",
    lead: "ひとつのリリースに三つのシステム。どれもまだ署名はありません。毎回テストしているのは macOS だけで、Windows と Linux はビルドして公開していますが、報告があるまでは未テストです。",
  ...
  unsigned: {
    title: "はじめて開くとき",
    lead: "このリリースはどのビルドも署名なしのため、どのシステムでも初回に警告が出ます。Mac 版の署名は次にやることです。それまでは以下のとおりに進めてください。一度だけで、次からは何も聞かれません。",
    mac: {
      title: "macOS",
      steps: [
        "`.dmg` を開き、**AI4Kanban** をアプリケーションフォルダにドラッグします。",
        "ダブルクリックします。*「AI4Kanban」にマルウェアが含まれていないことを Apple は確認できませんでした* と表示されるので、**完了**をクリックします。ここではまだ開きませんが、それで正常です。",
        "**システム設定 → プライバシーとセキュリティ**を開き、**セキュリティ**まで下にスクロールして、*「AI4Kanban」は Mac を保護するためにブロックされました* の横の**このまま開く**をクリックします。",
        "Touch ID かパスワードで認証し、もう一度**このまま開く**をクリックします。アプリが開き、次回からはそのまま起動します。",
      ],
    },
    windows: {
      title: "Windows",
