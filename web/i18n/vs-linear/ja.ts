...
const ja: VsLinearCopy = {
  meta: {
    ...
    description:
      "AI4Kanban と Linear を比較。リポジトリからエージェントチームを率いる AI プロジェクトマネージャーと、チームとエージェントが連携するプロダクト開発基盤の違いを解説します。",
    social:
  ...
  hero: {
    badge: "比較",
    title: "AI4Kanban vs.\nLinear",
    lead: "Linear は、チームがプロダクトの計画と開発を進めるための共有基盤です。AI4Kanban は、リポジトリの中からエージェントチームを率いる AI プロジェクトマネージャーです。一方は組織をつなぎ、もう一方は計画をコードから切り離さず、粗い依頼を実装可能な仕事へ整えます。",
    ours: {
      name: "AI4Kanban",
  ...
  comparison: {
    ...
    rows: {
      ...
      agentModel: {
        ...
        kanban: "選択したコーディング環境がボードを読み書きします。現在は Claude Code、Codex、Cursor、OpenCode、DeepSeek Harness、ZCode、Grok Build に対応しています。",
  ...
  wins: {
    ...
    ours: {
      ...
      anyHarness: {
        ...
        body: "ボードは独自のエージェントランタイムに縛られません。現在は Claude Code、Codex、Cursor、OpenCode、DeepSeek Harness、ZCode、Grok Build に対応し、オープンなファイル形式は他の環境でも利用できます。",
      },
      noSaas: {
