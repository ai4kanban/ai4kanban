...
<RequirementCascade caption="A goal splits into subtasks with their dependencies drawn. The arrows decide what Codex can run at the same time and what has to wait." />
...
No. AI4Kanban keeps the same small set of documents for the whole project, filed by who reads and writes them.

| File | Kept by | Keeps |
| --- | --- | --- |
| `readme.md` | The board | Shipped user-facing behavior |
| `goal.md` | The board | Where the project is headed |
| `decisions.md` | The planner | Current product decisions |
| `rejected.md` | The planner | Ideas that should not return |
| `redesign.md` | The planner | Design corrections to apply next time |

A card loads that project memory; the parts of the product it names are topics inside those files, not files of their own. Regular pruning merges duplicates and removes stale detail, so the agent reads a planning brief rather than the project's entire history.

<MemoryShot
  alt="AI4Kanban's memory page: the board's own record and the planner's memory in the sidebar, with settled decisions open"
  caption="Project memory stays in small, reviewable Markdown files, filed under whoever keeps them."
/>
...
<Shot
  src="configuration-agents-codex-v1"
  wash="emberMint"
  alt="AI4Kanban's Agents pane, with the builder set to Codex and its own model and reasoning effort"
