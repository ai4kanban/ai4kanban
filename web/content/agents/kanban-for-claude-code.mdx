...
**Project memory** contains decisions that should affect more than 1 task. AI4Kanban keeps a fixed, small set of Markdown files for the whole project, filed by who reads and writes them: the board keeps its own record, and each agent that remembers keeps a folder of its own.

| File | Kept by | What future Claude Code tasks learn from it |
| --- | --- | --- |
| `readme.md` | The board | User-facing behavior already shipped |
| `goal.md` | The board | Where the project is headed |
| `decisions.md` | The planner | Product decisions that still apply |
| `rejected.md` | The planner | Ideas that should not be proposed again |
| `redesign.md` | The planner | Design mistakes future work should avoid |

Each card names the parts of the product it changes. A new run receives the card plus that project memory, where those parts are topics inside the files rather than folders of their own. It does not receive every card or a replay of old chats.

<MemoryShot
  alt="AI4Kanban's memory page: the board's own record and the planner's memory in the sidebar, with settled decisions open"
  caption="Task history stays with the card. Shared decisions sit in small Markdown files, filed under whoever keeps them."
/>
...
<Shot
  src="configuration-agents-v1"
  wash="emberMint"
  alt="AI4Kanban's Agents pane, with the builder set to Claude Code and its own model and reasoning effort"
  caption="Each agent picks the coding tool and model it runs. The tool travels with the repository; the model stays on this computer."
/>
...
When automatic Git commits are enabled, each task gets its own branch and worktree. Independent tasks can run together; dependent tasks wait. Reviewed changes land on the target branch one at a time. If another task changed that branch, AI4Kanban rebases the next change onto it and lands it — and keeps retrying, without asking you, for as long as the branch keeps moving. A Git conflict gets a focused Claude Code resolution run, followed by another review before landing.
...
There is no fixed token number because task and memory size vary. Each run receives the current card, instructions for the current step, and the project memory a planning decision could come from. It does not load the whole board or replay AI4Kanban's chat history.
