...
**"What are we missing?"** — **new tasks**, found for you. The skill mines your planning
sources — the ones in `docs/kanban/config.md` — exactly as it mines an article you paste in:
it pulls out the concrete gaps, checks each against the board, your published docs,
`readme.md` and `rejected.md` so it won't repeat work, and writes the survivors as cards.
Strong signal only, so "nothing new" is a real answer. You approve, tweak, or drop each card.

Every card it writes is judged against `memory/goal.md` — the direction in your own words. The
file starts empty and stays optional: nothing on the board waits on it, and the skill never
writes the goal for you. It only marks how clear the goal looks in that file's `reviewed:`
field:
...
The board never asks you for a goal. Write one whenever you like, from the star in the top
row — with one, the cards it writes are judged against your direction; without one, they are
judged against the repository.
...
against the goal, code, docs, the Planner's memory, and open cards. Ideas that are already supported,
...
- **A card with open questions can still be built**, with a warning. It is built and reviewed,
  ...
  card still lands. **Resolve**, under the card's **open questions**, is the other way round: answer first, then build.
...
- **Skip a question you don't want to answer.** Open the card's **open questions** and press
  **Skip** on it: the card stays as it is, the question stays as a struck-through line with
  **Undo**, and once nothing else is open the delivery carries on. Deleting a question from the
  card file does the same.
...
- **A workflow that makes files gets no branch.** Product video, Slide deck, and any workflow of
  your own with **Use a Git worktree** off (the default for a new one; under **Advanced settings**
  in the `⋯` beside the workflow's name), works in your project and commits nothing, so several
  run at once. It is done when the files it records on the card exist and pass review; a change
  to your tracked files outside the board stops it instead. A Product video or Slide deck skips
  build and review altogether: it is made while planning, and you **Archive** the card to accept it.
- **The review is optional.** It is a separate paid run on every delivery, so
  **Review every build** on Configuration → General → Delivery turns it off for the board. The
  build then goes straight to landing; your own checks, the open-question hold and
  **Approve diffs before landing** still gate it.
...
**Gater** in Configuration → Board replaces that press. It is off by default. With it on,
every card that reaches ready is judged first by that agent: it reads the card against the same
standard the board writes cards to — an observable outcome, scope that says what to build, a
`## Todo` a builder can follow, nothing left open — and either passes it straight into a
delivery on the settings above, or sends it back to **Not ready** carrying the one question that
stopped it. Answer that question and the card refines, reaches ready again, and is judged again.

The Gater runs one card at a time, in the order **"what's next?"** would name them, and it
never takes a card that is blocked, recurring, a group's main card, or already in flight. It
judges as you would, so it is given the project's goal and the planner's `decisions.md` and
`rejected.md` on top of the card, and it runs on its own tool and model, set on its page in
Configuration → Board — so the judgment can run on a stronger model than the builds it lets
through. A run that fails or is stopped changes nothing: the card stays where it is. Turning the
switch on does not go back over the cards already sitting at ready — those are still yours to
press.
...
The Gater removes the press; **Decider** removes the wait. It is the other role in Configuration
→ Agents that is off by default, and the only agent whose switch asks before it goes on:
...
With it on, a card that ends its planning with nothing but questions waiting on you is answered
instead of handed back, and so is a delivery a review sent back with one. It answers from the
project's goal, the planner's `decisions.md` and `rejected.md`, and each question's own options
and recommendation, and takes the recommendation when none of those settles it. It never hands a card back, and there is
no limit on how many rounds it answers on one card.

What it chose stays on that card — the board marks it, and **What Decider chose for you** on the
card page lists every question, the option it took and the file it went on. None of it becomes a
lasting decision: a `decisions.md` records answers you gave.

Switching it off puts every card still carrying questions back to waiting on you; a card it
already answered and the board already built does not come back. Switching it on leaves the cards
already stopped exactly where they are. A run that fails changes nothing and is not started again
by itself — the card is yours again.

In a terminal it is `akb card decide 4`, which answers one card whether or not the switch is on.
See [the agents](/docs/agents) for the rest.
...
Say **"reject #4"** (rare). The card is dropped. When the rejection carries a lasting reason
not to propose the idea again, the skill adds a one-line "why not" to `rejected.md` and future
loops won't re-propose it — a duplicate or a card you have simply moved past leaves no note, so
memory stays worth reading. Say "don't record it" and nothing is written either way; on the
board, the Reject dialog's **Just discard** tick is that same answer — the card goes, no memory
is written, the reason is optional, and the idea can come back later.

## Keep it lean

Over time the memory grows — the board's own `readme.md` for what shipped, and the Planner's
`decisions.md`, `rejected.md` and `redesign.md` beside each agent's own. Ask the skill to
**prune** it: it compresses each to planning-useful summaries grouped by topic, so scans stay
cheap. The board itself stays small because finished work is a note, not a card.
