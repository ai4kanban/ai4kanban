...
**Build clear cards automatically** in Configuration → General → Delivery replaces that press.
It is off by default. With it on, every card that reaches ready is judged first by a gate
agent: it reads the card against the same standard the board writes cards to — an observable
outcome, scope that says what to build, a `## Todo` a builder can follow, nothing left open —
and either passes it straight into a delivery on the settings above, or sends it back to
**Not ready** carrying the one question that stopped it. Answer that question and the card
refines, reaches ready again, and is judged again.

The gate runs one card at a time, in the order **"what's next?"** would name them, and it
never takes a card that is blocked, recurring, a group's main card, or already in flight. It
runs on the planner's own tool and model, set in Configuration → Agents, so the judgment can
run on a stronger model than the builds it lets through. A gate run that fails or is stopped
changes nothing: the card stays where it is. Turning the switch on does not go back over the cards already sitting at
ready — those are still yours to press.

In a terminal it is `akb card gate 4`, which asks the same question about one card.

## Let the board answer your questions too

The gate removes the press; **Decider** removes the wait. It is a role in Configuration → Agents,
off by default, and it is the only agent whose switch asks before it goes on:
...
project's goal, the card's modules' `decisions.md` and each question's own recommendation, and
takes the recommendation when none of those settles it. It never hands a card back, and there is
