...
- **A workflow that makes files gets no branch.** Demo video, Slide deck, and any workflow of your own with
  **Use a Git worktree** off (the default for a new one; under **Advanced settings** in the `⋯`
  beside the workflow's name), works in your project and commits nothing, so several run at
  once. It is done when the files it records on the card exist and pass review; a change to your
  tracked files outside the board stops it instead.
