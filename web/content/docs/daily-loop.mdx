...
- **A workflow that makes files gets no branch.** Demo video, and any workflow of your own with
  **Advanced → Use a Git worktree** off (the default for a new one), works in your project and
  commits nothing, so several run at once. It is done when the files it records on the card exist
  and pass review; a change to your tracked files outside the board stops it instead.
