...
has a runtime of its own under the switch, so the judgment can run on a stronger model than
the builds it lets through. A gate run that fails or is stopped changes nothing: the card
stays where it is. Turning the switch on does not go back over the cards already sitting at
