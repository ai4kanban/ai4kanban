...
judges as you would, so it is given the project's goal and every module's `decisions.md` and
...
project's goal, every module's `decisions.md` and `rejected.md`, and each question's own options
