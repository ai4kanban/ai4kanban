...
`akb raw record-run <this card's id>` (+1 completed, card kept).
...
9. **Tag the lines a card is already building.** Run
   `akb raw list`, match the open cards against the unticked
...
12. Record the run: `akb raw record-run <this card's id>`.
