...
> down in April 2026. ai4kanban is a planning board your agent edits as
> plain files in your repo. Here's the honest difference, and what carries over.

- **AI4Kanban** — Plain Markdown in your repo. A planning board your agent
  edits.
...
sharpen work for your coding agent — ai4kanban gives you that as plain
...
ai4kanban is not that, and we'd rather tell you now than lose you three
sections in.

## 02 · Head to head — AI4Kanban vs. Vibe Kanban

Ten dimensions. A check is a clear win; a dash is a deliberate trade-off that
comes down to what you need. ai4kanban takes the lightness and planning
...
| Planning & refinement | A refine loop turns a rough idea into a ready, concrete task. | Minimal — the board mostly queues and tracks agent runs. | AI4Kanban |
| What it is on disk | Plain Markdown in your repo, in git. | A local SQLite database in a config directory. | AI4Kanban |
| Runs as | Just files — no server, nothing to keep alive. | A local web app (Rust backend + web UI) you start and keep running. | AI4Kanban |
| Setup | One prompt: a skill file and a small script. | npx vibe-kanban, plus each agent CLI installed and signed in. | AI4Kanban |
...
| Vendor lock-in | None — the board is files that travel with the repo. | Apache-2.0 and self-hosted, and a data export shipped before shutdown. | AI4Kanban |
| Who maintains it | Actively maintained. | Bloop shut down in April 2026; the original repo has since stalled. | AI4Kanban |
...
**AI4Kanban — the plan.** A board your agent reads and edits as plain Markdown
...
- **Runs many agents at once** — Its whole reason to exist: fan work out to
  ...
  worktree so they never collide. ai4kanban doesn't run agents at all.
...
running many agents; ai4kanban is a **planning board** one agent edits in
...
company attached — and that's exactly what ai4kanban is.
