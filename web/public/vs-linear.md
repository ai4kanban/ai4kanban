...
| Agent model | Your existing harness reads and writes the board; Claude Code, Codex, Cursor, OpenCode, DeepSeek Harness, ZCode, and Grok Build are wired up today. | Linear Agent plus installable app users, delegated issues, agent guidance, and a hosted MCP server. | Trade-off |
...
| Planning breadth | Cards, dependencies, priority, ROI, releases, and planning memory. | Issues, projects, cycles, initiatives, milestones, timelines, triage, insights, and customer requests. | Linear |
...
- **Bring your own harness** — The board is not tied to Linear Agent or one coding-agent
  integration. Claude Code, Codex, Cursor, OpenCode, DeepSeek Harness, ZCode, and Grok Build work today; the file format is open
  to any harness.
