...
| Agent model | Your existing harness reads and writes the board; Claude Code, Codex, Cursor, OpenCode, DeepSeek Harness, and ZCode are wired up today. | Linear Agent plus installable app users, delegated issues, agent guidance, and a hosted MCP server. | Trade-off |
...
- **Bring your own harness** — The board is not tied to Linear Agent or one coding-agent
  integration. Claude Code, Codex, Cursor, OpenCode, DeepSeek Harness, and ZCode work today; the file format is open
  to any harness.
