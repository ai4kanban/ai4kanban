...
- **This site counts two things.** A page was loaded, and a download button was pressed — so we can see whether the landing page works. No cookies, no accounts, nothing stored in your browser, and no identifier of any kind, so one visit is never tied to another. It is static files served by Cloudflare.
...
- **Anonymous usage reporting is on by default**, in the app and in the `akb` command. It sends which features are used and which of them fail — counts and the names of events, never a word you wrote — tied to a random install id and to nothing about you. It never carries your code, your card text, your project names, or your file paths. The app discloses it in a step you answer before your first board opens, and you can turn it off there, in **Configuration → General**, or with `akb telemetry off`.
- **Feedback is the one thing you can send us from the app, and only by pressing send.** Nothing is collected for it in the background and nothing is guessed. You choose whether to share what you typed, and separately whether to attach diagnostics — the card, the conversation, the run log, the environment — each listed with its size and readable in full before it goes, and each removable. Both permissions are for that one submission. It is one-way: there is no box for an address and we cannot write back.
- **Partner feedback is off unless you turn it on, and it is the only thing that sends us project files.** With it on, saying in a discussion that a spec missed what you meant — and ticking share on that message — lets the app gather what would let us reproduce it: the raw traces of that one refine's runs, and the project files those runs read. It is not previewed before it goes, it is kept until you ask for it back, and the app gives you a feedback number to quote when you do.
- **AI4Kanban Cloud is an invite-only preview** that relays a board's task events — a review to approve, a question to answer — to the people who can act on them, and the decisions they make, on servers we run in Frankfurt. On a board you share, a review reaches its members and a question reaches its owners. [What Cloud is](/cloud).
...
- **This site has two forms: booking a training session, and the contact form.** A booking sends us your name, your email address, the hour you picked and whatever you wrote about your project, so we can hold that hour and turn up to it. No account, no cookie, and nothing stored in your browser. You can cancel it yourself from the link in your confirmation, which releases the hour. A contact message sends us what you need help with, your email address and what you wrote, so we can answer you by email.
- **We do not sell your personal information**, and we do not share it for cross-context behavioral advertising.
- **You can ask us to access, correct, export, or delete** your personal information at any time at [support@ai4kanban.dev](mailto:support@ai4kanban.dev). Cancelling a training booking and disconnecting Slack or Lark are the removals you can make for yourself; everything else we do by hand.
...
The parts of the Service handle data very differently, so the sections that follow answer them one at a time: the site you are reading, the two forms on it, the app you install, and the Cloud preview a board's events can be relayed through. The rights and boilerplate sections after them apply to all of it.
...
- **We count page loads and download presses, and nothing else.** There is no analytics product on this site — no third-party script, no session recorder, no dashboard. The counter is a few lines of our own that post to `t.ai4kanban.dev`, the endpoint the app reports to, and the whole of what it sends is the table below. It answers one question: of the people who reach a page with a download button, how many press it.
- **No cookies.** The site sets no cookies of any kind, essential or otherwise, and stores nothing in your browser — no local storage, no identifier. There is no cookie banner because there is nothing to consent to.
...
- **No accounts.** There is nothing to sign up for on this site. The training page's booking form and the contact form are the two places it asks for your email address, and each is described in its own section below.
...
The training page at [`/training`](/training) sells one-to-one guidance around your own project. Booking an hour on it asks you for personal information once. The contact form, described after this section, is the only other place on the site that does.

You need no account, and there is nothing to sign in to. The page reads which hours the coach has open, you pick one, and what you send is stored on the same servers in Frankfurt that AI4Kanban Cloud runs on — in tables of its own, separate from any board, workspace or Cloud account. Nothing about a booking reaches a board, and nothing on a board reaches a booking.

### What a booking holds

| Field | Why we hold it |
| --- | --- |
| The hour you picked | It is the appointment. It also holds that hour against anyone else booking it. |
| Which session, and its price | So both sides agree what was booked before the coach writes to you. The price is settled by our server from the two figures printed on the page, never taken from your browser. |
| Your name | So the coach knows who is coming. |
| Your email address | Where the confirmation goes, and where the coach writes to arrange the meeting. It is the only way we can reach you: there is no account behind a booking. |
| Your timezone | The IANA name your browser reports, such as `Europe/Berlin`. Every time we show you or write to you is written in it. |
| What you wrote about your project | Optional, and read only by the coach, so the hour starts somewhere useful. |
| A booking reference | A short code, `TR-` and six characters, so you and the coach can name the same booking. |

Your browser tells us its timezone so the week is drawn in your own days and hours. That is read from the browser's own setting and nothing is stored in it — no cookie, no local storage, and no identifier that would let one visit be tied to another.

### What holds a booking against a rush of them

Submitting a booking is counted against a short-lived rate limit so nobody can fill the schedule from a script. What that counter is keyed on is a one-way hash of your IP address, salted with a value only our server holds; the address itself is never written down, and the counter resets on its own. It is a counter with a key, not a record of who visited.

### The manage link, and cancelling

Your confirmation carries a link that is the only way into your booking. It holds your reference and a random 32-byte token; we store only that token's SHA-256, so nobody with a copy of our database — including us — can rebuild a working link. Opening it shows your booking; cancelling takes two presses, releases the hour, and tells you and the coach that it is cancelled.

There is no way to reach a booking without that link. A wrong reference and a wrong token are answered identically, so the pair cannot be used to find out whose bookings exist.

### What a booking sends

Three or four messages, and no others. There is no mailing list and no follow-up marketing.

- **Your confirmation** — what you booked, when, in your own timezone, and the manage link. Sent from `training@ai4kanban.dev` through Resend, the same provider named in the subprocessor table above.
- **The coach's copy** — the same details plus what you wrote about your project, to the one mailbox we run. Replying to it answers you directly.
- **A cancellation notice** to each of you, if you cancel.

A message the provider will not take is retried; a booking is never undone because its email failed, and the page never tells you the mail arrived, only that the hour is held.

### Who can read a booking

The coach, and nobody else. Reading the booking records needs an admitted AI4Kanban Cloud account whose GitHub handle is on a short list held in the service's own configuration — being in the Cloud preview is not enough. The page that shows which hours are open answers everyone, and it answers with instants and nothing else: no name, no address, no project note, and no count of how many hours the schedule holds.

## Sending us a message

The contact page at [`/contact`](/contact) takes a support question or a request for custom agents. Like a booking, it needs no account, and what you send is stored on the same servers in Frankfurt, in a table of its own.

### What a message holds

| Field | Why we hold it |
| --- | --- |
| What you need | Support or custom agents, so the message reaches the right conversation. |
| Your email address | Where we reply. It is the only way we can reach you. |
| What you wrote | The message itself. |
| Your workflow | Only for custom agents: the description you gave of the workflow you want built. |

A submit is counted against a short-lived rate limit, keyed on two one-way hashes salted with a value only our server holds: one of your IP address and one of your email address. Neither the address nor the email is written into that counter, and it resets on its own.

The message is sent to `support@ai4kanban.dev` from `contact@ai4kanban.dev` through Resend, with your email address to reply to. That notice is the only mail it sends: you get no confirmation, and our reply is the answer. We keep the message until you ask us to delete it at [support@ai4kanban.dev](mailto:support@ai4kanban.dev).
...
- **Feedback is sent only when you press send.** The **Feedback** button beside Create task, and the block on New task that links a landed task, are the whole of it. Nothing is queued, nothing is collected in the background, and the usage-reporting switch does not reach it — it works either way. "Feedback you send us" below says what one submission carries.
- **Partner feedback is sent only when you have turned it on and ticked share on that message.** It is off out of the box, and turning it on is a page of terms you read first. "Partner feedback" below says what one submission carries.
...
**Feedback is the third thing, and the only one that carries words you wrote.** Usage reporting never does, on any event, whatever you type or build. Feedback does — that is what it is for — and it goes nowhere until you press send. The two are separate the whole way: separate permissions, a separate route, separate storage, and separate retention.

**Partner feedback is the fourth, and the only one that carries your code.** It is off unless you turn it on, and separate again the whole way down: its own switch, its own terms, its own route, its own private storage, and its own deletion key.
...
Two places send it, and nothing else does: the **Feedback** button in the app's top row, which takes feedback on anything at all; and the **Link a landed task** block on New task, which is one collapsed button until you open it. Opening it collects nothing either — it offers a search of your own archived cards, and two permissions, both off.

It goes to `t.ai4kanban.dev`, the same endpoint the anonymous numbers go to, on a route of its own. It is stored apart from them, in its own tables, and is never written into the daily archive those events are.

**One submission carries only what you agreed to, and each of these is its own answer:**

| What | When it is sent |
| --- | --- |
| **What you wrote** | Always — it is the submission. From New task it is the description you typed, and it is sent only if you ticked **Share this description**. |
| **The number of the archived card you linked** | Only when you linked one. Its title and its text are not this — they are an attachment below. |
| **Diagnostics** | Only when you separately tick **Attach diagnostics**. Four parts at most — the card's own text, the conversation held on it, the run log of the build that landed it, and your operating system, processor architecture, app version and coding-agent name. Each is listed with its size, readable in full before it goes, and removable on its own. Nothing else on your machine is read. |
| **Your install id** | Only when your machine already has one — that is, when usage reporting is on. Feedback never creates one and never restores a forgotten one, so feedback from a machine with reporting off carries no identifier at all. |
| **Your country** | Worked out from the address the submission arrived from, and stored in its place. The address itself is not. |

**What it never carries:** an address or any way to reach you — there is no field for one, and we do not attempt to work out who sent a submission; anything you did not tick; anything about a card you did not link; and anything on your machine outside the four parts above.

**Both permissions are for that one submission.** Nothing is remembered. Cancelling the block clears the link and both ticks and keeps what you typed, and opening it again starts from nothing ticked.

**It is one-way.** We read feedback; we cannot reply to it, and nothing we store would let us. If you want an answer, email [support@ai4kanban.dev](mailto:support@ai4kanban.dev) instead.

**How long it is kept.** The words you wrote are kept **indefinitely** — they are the whole reason the feature exists, and a note deleted at 90 days would be a report we could not act on a year later. Diagnostic attachments are deleted after **90 days**, on the same sweep the raw usage events get; deleting them leaves the words beside them untouched. To have feedback you sent deleted, email [support@ai4kanban.dev](mailto:support@ai4kanban.dev) with your **install id** — the same id, and the same request, that deletes your events. Feedback sent from a machine with reporting off carries no id, so there is nothing we could find it by.

### Partner feedback

**It is off out of the box, and turning it on is a page you read first.** The switch is **Configuration → General → Partner feedback**. The page it opens says what is shared, that it is kept long-term, that the analysis runs on the AI service you have set up, and how to have it deleted. Turning it off takes no page and stops any further collection at once.

**It is asked for one message at a time.** With the switch on, it still does nothing until you say in a discussion that a spec missed what you meant, link the card it is about, and tick **Share this description with the AI4Kanban team** on that message. The switch alone shares nothing; the tick alone, on a machine that never opted in, shares nothing either.

**What one submission carries.** The `feedback` agent on your machine works out which refine of that card went wrong, and the app gathers:

| What | Detail |
| --- | --- |
| **What you wrote** | The problem as you described it in the discussion. |
| **The number of the card you linked** | And nothing else about your board. |
| **The raw traces of that one refine's runs** | As your own coding agent recorded them — including the instructions each run was given, which survive nowhere else. Only the runs of the refine that was settled on. |
| **The project files that refine read** | The files, at the version that refine saw them where your repository can still answer for it, and this checkout's copy otherwise — which the submission says. Only files the agent could evidence as read, and never the board's `.env`. |
| **What could not be established** | A trace already cleaned up, a file no longer there, a run too old to record its version. Written down rather than left out. |

**What it never carries.** An address or any way to reach you; anything about a card you did not link; anything from a refine other than the one you were describing; and your install id — this one does not use it, which is why it works with usage reporting off.

**It is not previewed.** The material is gathered for you and sent when it is ready. That is the trade the terms page states plainly, and it is why the switch is off until you turn it on.

**How long it is kept, and how to have it deleted.** It is kept until you ask for it back — a case we cannot reproduce a year from now is a case that taught us nothing. When a submission lands, the app shows you a **feedback number** and the address to send it to. Email [support@ai4kanban.dev](mailto:support@ai4kanban.dev) with that number and we delete the material that submission sent **and anything we had already made from it**. The number is minted on your machine and does not depend on an install id, so a machine with usage reporting off can still have its submissions deleted.
...
**A workspace can hold more than one person; what is yours alone stays yours alone.** A workspace answers the people an owner has added to it, in one of two roles — an owner, who runs its name, its members and its machines, and a member, who does everything else on the board. A workspace's board, its history, and the events its cards raise answer every member of it. Every other record described below — your account, your admission, your invite request, your Slack and Lark connections, what you have asked to be told about, and the boards and events of a board kept on your own machine — belongs to one signed-in account, and Cloud answers that account alone.
...
Turning Cloud on for a board sends Cloud one **event** per task that needs a person: a task that reaches `ready`, or one that raises a question only a person can answer. An event holds that task's **number, title, release, and current revision**, the decision it asks for, and — when it is a question — **each question with its options and the board's recommendation**.

On a board you share, one event belongs to the workspace rather than to the machine that raised it, and **who it reaches is worked out each time it is read** from who is in the workspace then: a task ready for review reaches its members, and a question reaches its owners. Somebody an owner added since is included on their next read; somebody removed since is dropped. Nobody outside the workspace is ever told.
...
Once you act on an event, Cloud also holds **the decision you made and the answers you gave** — attributed to you on the workspace's own record of who changed what — and **how the run it started ended**. Where the event was delivered is recorded against it too, once **for each chat destination** it went to, so a board two people share leaves one record per chat rather than one per person.
...
### What you have asked to be told about

On a board you share, **whether you want to be told at all, and which release you want to be told about**, are held in the workspace rather than on your computer — so they follow you to every machine you open that board on, and so Cloud can work out who a card's decision should reach. That is two values and nothing else: a switch, and either one release's name or *every release*. Turning the switch off stops Cloud telling you anything about that board and leaves the rest of the workspace untouched.

An owner adding you sets both once — the switch on, watching the version the team is shipping now — and either of you can change them afterwards. They are deleted when your membership is, and when the workspace is.

### Which message in a chat is the card's

A task keeps one message in each chat it was sent to, and its events are logged in the thread under it. So Cloud holds **which message that is**: the board, the task number, the chat, and **the chat's own identifier for that message**. On a board you share, that is one message per chat the board's people connected — two of them reading the same channel share one message rather than getting two. It holds none of the message's words, and nothing about who read it or replied to it. It is deleted when the task's last event is, and it goes with the board and the account it belongs to.
...
| --- | --- | --- |
| **Cloudflare, Inc.** | Compute and hosting — the API, the usage-reporting endpoint, the database behind it and the object storage holding the daily event archive, and the static site — and the Email Routing that delivers `support@ai4kanban.dev` | Global network |
| **Supabase, Inc.** | The database and sign-in | `eu-central-1`, Frankfurt |
| **Resend (Plus Five Five, Inc.)** | Sending the answer to an invite request, the request notice, and the mail the site's two forms send | United States |
...
You can connect **Slack** or **Lark** — which is 飞书 in China and Lark internationally — so that a question or a review request is posted into a channel, a group, or a direct message you already read, and decided from there. **One connection per chat belongs to your account**, and every board you turn Cloud on for shares it. A connection is never a workspace's: on a board you share, each person is reached in their own connected chat, and a chat two of them picked gets one message. You can connect both at once. You connect each one and you disconnect it; nobody else can.
...
**Everything else we remove by hand**, on a request to [support@ai4kanban.dev](mailto:support@ai4kanban.dev). What that removes is **your whole account and everything under it**: your Cloud account row and your admission, your board rows with their names, every machine record those boards registered, every event with its questions, decisions, answers and outcomes, which message in a chat each task is, your Slack and Lark connections, your invite request, and your membership of every workspace you are in with what you asked to be told about in it. Your sign-in record goes alongside them, as its own step in Supabase rather than by the same operation.
...
- **Read and act on feedback you send us** — the words you chose to share, and any diagnostics you chose to attach, so we can find what went wrong and fix it. We do not use feedback to identify you, and we cannot reply to it.
- **Reproduce and fix what a partner feedback submission reports** — the traces and project files you chose to share, so we can see where a spec misread what you meant and improve how specs are written. We do not use it to identify you.
- **Understand whether the product works** — count how the app and the `akb` command are used and where they fail, and how many people who reach a page with a download button press it, from the anonymous reports described above, so we can decide what to build and what to fix. We read these as totals, never as one machine's or one visitor's story, and we do not attempt to work out who an install id belongs to.
...
- **Arrange a training session you booked** — hold the hour, confirm it to you, give the coach what they need to turn up to it, and let you cancel it.
- **Answer you** — handle a support message, a contact form message, or a data request you send us.
...
- **Contract** — running the Service you asked for, including holding the Cloud records of the account that turned Cloud on, answering the invite request you made, arranging the training session you booked, and answering a message you sent through the contact form.
- **Legitimate interests** — keeping the Service secure and working, preventing abuse, including the rate limits on the booking and contact forms, telling an account in the preview that the preview is ending or that these pages have materially changed, understanding how the app is used from anonymous usage reporting, which you can turn off at any time and which we disclose before your first board opens, and the two counts this site keeps, which carry no identifier and so are about no one in particular.
...
- **Feedback you sent us** — the words you wrote are kept indefinitely. Any diagnostic attachments are deleted 90 days after the submission, and deleting them leaves the words untouched. Neither is written into the daily archive. Both are deleted on request, quoting the install id the submission carried; one sent from a machine with reporting off carries none and cannot be found.
- **Partner feedback you sent us** — kept until you ask for it back, in private storage of its own that holds nothing else. Deleted on request, quoting the feedback number the app showed you; the deletion covers the submission and anything we had already made from it.
- **The rest of app use** — nothing is retained, because nothing else is collected. Apart from usage reporting and feedback you chose to send, a board you have not turned Cloud on for sends us nothing to retain.
...
- **What you have asked to be told about in a workspace** — the switch and the release described above, kept while you are in that workspace, and deleted when your membership is or when the workspace is.
...
- **A training booking** — the fields in the table above, kept for 24 months after the session, so both sides have a record of what was arranged and paid for. Cancelling releases the hour at once and marks the booking cancelled; the row itself stays for the same 24 months and is deleted on request, quoting your booking reference. The rate-limit counter behind a submit is keyed on a salted hash and expires within the hour.
- **A contact form message** — the fields in the table above, kept until you ask us to delete it. The rate-limit counter behind a submit is keyed on salted hashes and resets on its own.
...
Four things you can do without asking us: **turn anonymous usage reporting off**, which stops it at once and deletes what is queued on your machine; **turn partner feedback off**, which stops any further collection at once; **disconnect Slack or Lark**, which deletes that connection at once; and **turn Cloud off for a board**, which stops it sending anything more. Feedback needs nothing turned off: it is sent only when you press send. A copy, a correction, or a removal of what we already hold is a request to us — for partner feedback, quoting the feedback number the app showed you.
...
- **Other information you choose to provide** — the part of a card an event carries (its title, its opening paragraph, its review notes, and its questions), the decisions and answers you record against it, feedback you press send on and anything you chose to attach to it, the traces and project files a partner feedback submission carries, and anything you write to us in an email.
