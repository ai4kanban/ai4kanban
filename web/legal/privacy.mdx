...
- **Booking a training session is the one form on this site.** It sends us your name, your email address, the hour you picked and whatever you wrote about your project, so we can hold that hour and turn up to it. No account, no cookie, and nothing stored in your browser. You can cancel it yourself from the link in your confirmation, which releases the hour.
...
The parts of the Service handle data very differently, so the sections that follow answer them one at a time: the site you are reading, the one form on it, the app you install, and the Cloud preview a board's events can be relayed through. The rights and boilerplate sections after them apply to all of it.
...
- **No accounts.** There is nothing to sign up for on this site. The training page's booking form is the one place it asks for your email address, and it is described in its own section below.
...
The training page at [`/training`](/training) sells one-to-one guidance around your own project. Booking an hour on it is the only thing anywhere on this site that asks you for personal information, and it asks once.
...
- **Contract** — running the Service you asked for, including holding the Cloud records of the account that turned Cloud on, answering the invite request you made, and arranging the training session you booked.
- **Legitimate interests** — keeping the Service secure and working, preventing abuse, including the rate limit on the booking form, telling an account in the preview that the preview is ending or that these pages have materially changed, understanding how the app is used from anonymous usage reporting, which you can turn off at any time and which we disclose before your first board opens, and the two counts this site keeps, which carry no identifier and so are about no one in particular.
