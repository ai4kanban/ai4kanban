...
- [x] **Not tied to a single agent.** The board is decoupled from agent runtimes. Claude
  Code, Codex, Cursor, OpenCode, and DeepSeek Harness all run it today, and the next one
  is whichever people ask for.
...
task cards. Starting runs needs your coding agent — Claude Code, Codex, Cursor, OpenCode,
or DeepSeek Harness — on the machine, same as from a terminal.
...
(Claude Code) and `.agents/skills/kanban/` (Codex, Cursor, OpenCode, and DeepSeek
Harness) — saying the board
...
- [ ] **More agent runtimes** (in progress): Claude Code, Codex, Cursor, OpenCode, and
  DeepSeek Harness are supported today. The next one is whichever people ask for.
