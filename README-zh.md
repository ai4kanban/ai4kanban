...
- **管理任务生命周期。** Agent 拆解任务、安排依赖和优先级、规划版本，并将就绪任务交给编码
  Agent 执行。
- **一路交付到合入。** 点一次 Implement，任务就会在独立的 git worktree 中开发、按卡片要求
  自查、修正 review 发现的问题，并作为一个 commit 合入你的分支。
- **人不在电脑前也能找到你。** 待确认的 review 和只有你能回答的问题会汇总到应用的通知中心，
  并推送到 Slack；在消息里就能通过审批、回答问题。
...
应用支持简体中文和英文，启动时跟随系统语言。看板内容也会用你选定的语言书写——卡片、待答
问题和验收条目都在内。
