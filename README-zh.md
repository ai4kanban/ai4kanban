...
- **本地存储，可自由选择 Agent。** 看板以 Markdown 文件存放在 `docs/kanban/`，用 Git 管理
  ...
  ZCode 和 Grok Build。
- **默认开启匿名使用情况上报。** 只上报哪些功能被使用、哪些出现失败，绝不包含代码、卡片内容、
  项目名称或文件路径。应用会在你打开第一个看板前先作出说明，你可以在那里关闭，也可以之后在
  **配置 → 通用**中关闭，或运行 `akb telemetry off`。全部事件与字段见
  [隐私政策](https://ai4kanban.dev/privacy)。

## 本地看板与 Cloud 看板

新安装默认从**本地看板**开始：卡片、记忆、发布和配置都是 `docs/kanban/` 下的 Markdown 文件，
由 Git 管理版本，看板内容不会离开这台机器（匿名使用情况上报只发送计数和事件名称，绝不包含
看板内容）。启动页把它放在最显眼的位置——**新建本地看板**和
**打开本地看板**。
