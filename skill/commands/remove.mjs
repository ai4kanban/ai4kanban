...
import { die, warn, rel, TODO, MEMORY, ARCHIVE } from '../lib/paths.mjs'
import { bumpMetric } from '../lib/metrics.mjs'
import { walkMd, locate, enclosingGroupRoot, markSubtask, archiveDest } from '../lib/cards.mjs'
import { stripReadmeRefs } from '../lib/readme.mjs'
import { parseFrontmatter, frontmatterEnd, frontmatterField } from '../lib/frontmatter.mjs'
import { memoryTargets } from '../lib/memory.mjs'
...
export function cmdRemove(id, metric) {
  ...
  if (removedRefs.length) console.log(`  dropped ${removedRefs.length} README ${removedRefs.length === 1 ? 'entry' : 'entries'}`)
  ...
  printHandoff(id, metric, cardMeta, findMentions(id))
...
function quoteLine(text, width = 96) {
...
function printHandoff(id, metric, meta, mentions) {
  ...
  console.log(`  1. ${kind.what} — follow "${kind.section}" in the skill`)
  for (const t of targets) {
    console.log(`       file    ${rel(t.file)}`)
    if (!kind.topics) continue
    const topics = t.topics.map((x) => `"${x.name}" (${x.entries})`).join(', ')
    console.log(`       topics  ${topics || '(none yet — this note starts the first one)'}`)
  }
  if (targets.length > 1) {
    console.log('       both, because the card named two modules — one note each, in its own words')
  }
  ...
  for (const m of mentions) {
    console.log(`       ${rel(m.file)}:${m.line}  (${m.where})`)
    console.log(`         ${quoteLine(m.text)}`)
  }
  if (mentions.some((m) => m.where !== 'body')) {
    console.log('     A mention in frontmatter is the script\'s to rewrite, not yours:')
    console.log('     `update-questions <id> --update <n> "..."` (see help).')
  }
}
...
function printEpitaph(id, relPath, text, alsoRemoved) {
  ...
  if (lines.length > shown.length) {
    console.log(`  | ... ${lines.length - shown.length} more line(s) — \`git show HEAD:${relPath}\` for the rest`)
  }
  console.log('  `--')
  if (alsoRemoved.length) {
    console.log(`\n  the folder took ${alsoRemoved.length} subtask card(s) with it:`)
    for (const f of alsoRemoved) console.log(`    ${f}`)
    console.log('    (in git history — `git show HEAD:<path>`)')
  }
}
