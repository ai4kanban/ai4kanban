...
import { die, rel, TODO, ARCHIVE } from './paths.mjs'
...
export function walkDirs(dir, acc = []) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
...
export function enclosingGroupRoot(file) {
...
export function markSubtask(rootFile, id, action) {
...
export function archiveDest(found) {
...
export function isRecurringCard(found) {
