...
```
akb raw release new v2                # create a release before assigning cards to it
akb raw release new v2 --goal ".."    # create it with an optional goal
akb raw release new v2 --fill         # add high-priority, unblocked, non-root cards with no release
akb raw release goal v2 ".."          # change its goal; "" clears the goal
akb raw release list                  # show ship order, goals, card counts, and invalid references
akb raw update 14 --release v2        # assign a card; --release "" removes its assignment
akb raw release close v2              # record a release that shipped
akb raw release drop v2               # remove a release that will not ship
```
...
- **Assign or remove a card**: Use `akb raw update <card-id> --release <release-id>` to assign a card and `akb raw update <card-id> --release ""` to remove it from that release without deleting the card.
...
- **Reorder or rename carefully**: Edit `docs/kanban/releases.md` by hand, run `akb raw release list` to find cards that still use an old identifier, and update each affected card explicitly.
