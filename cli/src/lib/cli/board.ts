...
export function buildBoardProgram(cli: BoardCliOptions): Command {
  ...
  move('plan')
    .argument('<move>', 'new, save, drop or migrate')
    .summary('save a discussion plan in this checkout')
    .description('Write a temporary draft, then use plan new --title "…" --body-file <file>. Update with plan save --path <plan> --body-file <file>. Withdraw one with plan drop --path <plan> so it is not handed off. Pass --workflow <id> on either to name the workflow the plan fits. Success confirms the body and discussion link were saved. plan migrate moves legacy board plans into .akb/.')
    .option('--body-file <path>', 'the complete plan body')
    .option('--path <path>', 'the existing plan to update or withdraw')
    ...
    .option('--workflow <id>', 'the workflow this plan fits, from `akb workflow list`')
  ...
  move('discussion')
    ...
    .description(
      'A board holds many discussions at once (#496), each with its own transcript, agent session and ' +
        "plans. `discussion list` says which are going and what each is called; `discussion new` opens " +
        'one; `discussion title --title "…"` names one; `discussion archive` takes one out of the list, ' +
        'leaving its transcript on this machine. Told no --id, `title` and `archive` act on the ' +
        'discussion this turn is answering.',
    )
    .option('--id <id>', 'the discussion, where it is not the one this turn is answering')
    .option('--title <title>', 'what to call it')
    .action(async function (this: Command, sub: string) {
