...
export function buildBoardProgram(cli: BoardCliOptions): Command {
  ...
  move('plan')
    ...
    .description('Write a temporary draft, then use plan new --title "…" --body-file <file>. Update with plan save --path <plan> --body-file <file>. Pass --workflow <id> on either to name the workflow the plan fits. Success confirms the body and discussion link were saved. plan migrate moves legacy board plans into .akb/.')
