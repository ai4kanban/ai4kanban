 ...
 */
export function startCloudServer(boardDir = KANBAN): void {
  const held = state()
  ...
  held.timer = setInterval(() => {
    void renewCloudClaims(boardDir).catch(() => {})
    ...
    if (!state().live?.joined()) void catchUpCloudRequests(boardDir).catch(() => {})
    ...
    void readBoardServer(boardDir).catch(() => {})
  }, RENEW_MS)
  held.timer.unref?.()

  held.starting = (async () => {
    await catchUpCloudRequests(boardDir)
    ...
    held.live = connectCloudLive({
      ...
      // nothing.
      onReady: () => void catchUpCloudRequests(boardDir).catch(() => {}),
      onHint: (payload) => {
        const id = payload.requestId
        if (typeof id === 'string' && id) void takeCloudRequest(id, boardDir).catch(() => {})
      },
    })
  })()
...
async function claimTheBoard(boardDir: string): Promise<void> {
  const held = state()
  if (!wantsServerHere(boardDir)) return
  if (held.askedAt && Date.now() - held.askedAt < RENEW_MS) return
  held.askedAt = Date.now()
  try {
    const server = await readBoardServer(boardDir)
    if (!server.attached) await attachBoardServer(false, boardDir)
  } catch {
