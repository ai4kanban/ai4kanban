...
import { namedById } from '../solution'
...
/** What a non-English title costs in the filename. Nothing where a card is named off its id
 *  alone (#507): there `--slug` is refused, so telling a run to pass it would be telling it
 *  to make a call the board turns down. */
const slugRule = (): string =>
  namedById() ? '' : ', and a title that is not English needs `akb raw create --slug` a short English slug'
