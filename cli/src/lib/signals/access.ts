...
export async function signalsAccess(): Promise<SignalsAccess> {
  ...
  if (account.error) {
    return { open: false, why: `Triage is open to invited Cloud accounts, and Cloud could not be reached to check: ${account.error}` }
  }
  switch (account.state) {
    ...
    case 'not-admitted':
      return {
        open: false,
        why: 'Triage is open to invited Cloud accounts. Request an invite in the AI4Kanban app, under Configuration → Notifications.',
      }
    case 'expired':
      return { open: false, why: `Your Cloud sign-in has expired. ${SIGN_IN}` }
    default:
      return { open: false, why: `Triage is a Cloud preview. ${SIGN_IN}` }
  }
}
