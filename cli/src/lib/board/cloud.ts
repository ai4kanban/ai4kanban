...
export type OpenRefusal = 'signed-out' | 'not-a-member' | 'unreachable' | 'refused'
...
function refusalOf(call: { ok: false; error: string; code?: string }): {
  ...
  if (call.code === 'not_a_member' || call.code === 'not_yours' || call.code === 'not_found') {
    // Cloud answers a deleted workspace and one this account is not in with one code on
    // purpose, so nothing leaks whether a workspace ever existed (#317). One sentence has to
    // be true of both — and since #376 a workspace has members, so the way in is an owner
    // adding this account rather than signing in as the one that owns it.
    return {
      ok: false,
      reason: 'not-a-member',
      error:
