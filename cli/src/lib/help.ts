  ...
  },
  {
    ...
    legacy: [
      [
        ...
        [
          'have the board run that action on this card by itself, rather',
          'than starting a run here. An unblocked card is started on the',
          "board's next tick; a blocked one the moment every card it waits",
          'on has left the board (archived or rejected — either way that',
          'card is gone). The mark comes off as the run starts, so it',
          'fires once. --notes ".." rides along and reaches that run.',
          'Scheduling is what to use for work that must outlive this',
          'session: it lives in the card, so it survives a restart, a',
          'reboot and a clone, and the board starts one scheduled run per',
          'tick instead of all of them at once.',
          'A card holds one schedule at a time, so a second one replaces',
          'the first, and a rough card gets a refine schedule by default',
          'when it first becomes blocked. --clear takes it off; changing',
          'one non-empty blocker list into another leaves it off.',
          'Refused only when the action would not move the card.',
        ],
      ],
    ],
  },
